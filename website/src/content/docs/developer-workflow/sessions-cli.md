...
Credential recovery or cleanup failures are an exception: the CLI reports an error on stderr instead of treating the rejection as fully handled. A reused pending credential is retained if a later request fails or is rejected, because that does not settle the earlier operation. For CLI operations, repeat the same command and operation key under the same caller identity to reconcile an unknown outcome; a changed command or identity is not an equivalent retry. Legacy file pending artifacts that do not bind their original destination cannot be retried automatically, even with the same key; use the reported protected path for manual recovery. If protected-file cleanup fails, the error reports the remaining recovery location rather than claiming the secret was removed.

If the GUI loses the response to profile creation or credential rotation, the Host may already have committed the change. OpenWaggle retains the protected credential and reports the unknown outcome, operation reference, and recovery path without displaying the secret. A later rejected GUI retry also preserves an earlier pending credential. Keep the error details and protected file until the Host outcome is confirmed and the credential recovered; do not delete the file just because a retry was rejected. GUI recovery is not automatic: a new GUI request uses a new operation key, and a CLI request uses a different caller identity, so neither replays the original GUI operation. An accepted creation whose local credential installation fails is reported separately as created, with its recovery path.

For an existing, non-revoked profile, you can explicitly recover through a new rotation: close any failed-create dialog, collapse and reopen **Restricted CLI profiles** to refresh the Host's list, then choose the intended profile's **Rotate** action and confirm. When one compatible pending store credential exists, the accepted rotation installs it. This is a new operation, not a replay; old clients disconnect and must reconnect using the stored credential. The earlier protected artifact remains available for its original operation's exact-key retry or manual recovery, but a later new rotation will not adopt those already-installed bytes again; it generates a fresh credential. A revoked profile, multiple eligible pending artifacts, or corrupt credential data prevents the recovery path. Keep unresolved artifacts and their reported locations instead of deleting them or retrying blindly.
