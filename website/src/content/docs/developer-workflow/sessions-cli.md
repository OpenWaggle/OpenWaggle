...
Credential recovery or cleanup failures are an exception: the CLI reports an error on stderr instead of treating the rejection as fully handled. A reused pending credential is retained if a later request fails or is rejected, because that does not settle the earlier operation. For CLI operations, repeat the same command and operation key under the same caller identity to reconcile an unknown outcome; a changed command or identity is not an equivalent retry. If protected-file cleanup fails, the error reports the remaining recovery location rather than claiming the secret was removed.
