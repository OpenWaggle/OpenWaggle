...
export function createOrchestrationEngine(options: OrchestrationEngineOptions): OrchestrationEngine {
  ...
  function buildInitialState(definition: OrchestrationRunDefinition): MutableRunState {
    ...
    return {
      ...
      status: "running",
      startedAt: nowIso(),
      maxParallelTasks: Math.max(1, Math.floor(definition.maxParallelTasks ?? DEFAULT_MAX_PARALLEL_TASKS)),
      tasks,
      taskOrder,
      outputs: {},
    };
  ...
  function restoreState(snapshot: OrchestrationRunRecord): MutableRunState {
    ...
    for (const taskId of snapshot.taskOrder) {
      ...
      tasks.set(taskId, {
        ...
        output: task.output,
        status:
          task.status === "running" || task.status === "retrying" ? "queued" : task.status,
        retry: task.retry,
  ...
  async function runInternal(
    ...
    const enqueueIfReady = (taskId: string): void => {
      ...
      if (!task.dependsOn.every((dependencyId) => state.tasks.get(dependencyId)?.status === "completed")) {
        return;
      }
      queue.push(taskId);
      ...
      try {
        const response = await options.workerAdapter.executeTask(taskToDefinition(task), {
          ...
          reportProgress: (payload) => {
            void emit({ type: "task_progress", runId: state.runId, taskId, at: nowIso(), payload });
          },
      ...
      while (true) {
        ...
        if (running.size === 0) {
          ...
          const pendingTask = [...state.tasks.values()].find(
            (task) => task.status === "queued" || task.status === "retrying" || task.status === "running",
          );
          if (pendingTask) {
            enqueueAllReadyTasks();
            if (queue.length > 0) {
...
function shouldRetry(policy: Required<OrchestrationTaskRetryPolicy>, attemptNumber: number): boolean {
...
function toRunSummary(snapshot: OrchestrationRunRecord): RunSummary {
  const failedTaskIds = snapshot.taskOrder.filter((taskId) => snapshot.tasks[taskId]?.status === "failed");
