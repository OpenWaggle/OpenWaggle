...
function useExtensionSyntaxResult({
  ...
  const [result, setResult] = useState(() => createPlainExtensionSyntaxResult({ source, language }))
...
export function SyntaxBlock({
  ...
  const result = useExtensionSyntaxResult({ syntax, source, language, path })
...
export function SourceView({
  ...
  const sourceLineCount = useMemo(() => source.split('\n').length, [source])
  ...
  return (
    <section aria-label={ariaLabel}>
      ...
      >
        ...
        >
          {result.lines.slice(start, end).map((line, visibleIndex) => {
