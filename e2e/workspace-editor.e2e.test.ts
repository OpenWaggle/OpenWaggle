...
async function beginReadableMeasurement(page: Page, label: string, marker: string) {
  await page.evaluate(
    ({ targetLabel, textMarker }) => {
      const startedAt = performance.now()
      const recordIfReadable = () => {
        const target = [...document.querySelectorAll('[aria-label]')].find(
          (element) => element.getAttribute('aria-label') === targetLabel,
        )
        if (!target?.textContent?.includes(textMarker)) return false
        Reflect.set(window, '__openwaggleE2eReadableMs', performance.now() - startedAt)
        return true
      }
      const observer = new MutationObserver(() => {
        if (recordIfReadable()) observer.disconnect()
      })
      observer.observe(document.body, { childList: true, subtree: true })
      recordIfReadable()
    },
    { targetLabel: label, textMarker: marker },
  )
}
...
test('workspace files stay review-first, edit reliably on demand, and remain responsive', async () => {
  ...
  try {
    ...
    await openWorkspaceFile(page, 'src/main.ts', () =>
      beginReadableMeasurement(page, 'Source for src/main.ts', 'export const result'),
    )
...
test('a 1 MiB source file paints before tokenization and keeps bounded work', async () => {
  ...
  try {
    ...
    await expect(result).toBeVisible()
    await installLongTaskObserver(page)
    await beginReadableMeasurement(page, `Source for ${relativePath}`, 'export const value')
    await result.click()
