...
export async function captureHiddenWindowPresentation(application: ElectronApplication) {
  const windowId = await application.evaluate(
    ({ BrowserWindow }) =>
      new Promise<number>((resolve, reject) => {
        const window = BrowserWindow.getAllWindows()[0]
        if (!window) throw new Error('Hidden presentation window is missing.')
        ...
        const timeout = setTimeout(() => {
          ...
          reject(new Error('Hidden compositor did not present a frame.'))
        }, 5_000)
