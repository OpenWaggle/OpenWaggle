...
test('session resources stay scoped while inline images and the gallery navigate', async () => {
  ...
  try {
    ...
    await expect(summary).toBeVisible()
    await inlineUserImage.hover()
    await page.getByRole('button', { name: 'Branch from message', exact: true }).first().click()
    await expect(page.getByText('Keep context from the abandoned branch?')).toBeVisible()
    ...
    await expect(page.getByText('Keep context from the abandoned branch?')).toBeVisible()
    await app.captureEvidence('session-summary-viewer-preserves-branch-prompt')
    await page.getByRole('button', { name: 'Cancel', exact: true }).click()
    await expect(page.getByText('Keep context from the abandoned branch?')).toHaveCount(0)
    await page.locator('header').getByRole('button', { name: 'Hide Session Summary' }).click()
    await expect(summary).toHaveCount(0)
    await inlineUserImage.click()
    await expect(page.getByRole('dialog', { name: 'Image viewer: user-reference.png' })).toBeVisible()
