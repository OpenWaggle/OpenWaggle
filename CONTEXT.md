...
The product rule that Codex GUI `26.831.21537`, build `7579`, audited on 2026-09-04, supplies the version-pinned observable interaction baseline. OpenWaggle matches that behavior for capabilities it truthfully owns, with deviations only where Codex has no equivalent, OpenWaggle's domain requires one, or a materially better solution is chosen explicitly. A newer Codex build requires another behavior audit before it replaces this baseline.
