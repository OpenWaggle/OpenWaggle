...
- [Guard `git push` with an agent command gate](#guard-git-push-with-an-agent-command-gate)
...
## Guard `git push` with an agent command gate

Use this when an agent runtime is allowed to run Git commands in a repository that already uses fallow.

The pattern is a local agent gate, not a Git hook. The agent runtime intercepts its own shell commands before execution. When an agent tries `git commit` or `git push`, the gate runs:
...
Agent runtimes that surface blocked-command stderr can show the structured findings
(including `_meta.docs` links and `actions`) so the agent can fix the code and retry.
...
Manual agent-hook files are tool-specific. Prefer `fallow setup-hooks` only for
tools it explicitly supports, and keep this repository's canonical agent policy
in `AGENTS.md` and `.agents/`.

### Fallow gate behavior

When an agent hook is installed, prefer the script written and maintained by
fallow itself; the canonical source is [`crates/cli/src/setup_hooks/fallow-gate.sh`](https://github.com/fallow-rs/fallow/blob/main/crates/cli/src/setup_hooks/fallow-gate.sh).
...
Keep `fallow audit` in CI alongside any local gate. Tool-specific hooks are
reinforcement layers, not replacements for repository or server-side enforcement.
...
Removes the fallow-gate handler from supported agent settings (preserving any
other handlers in the same matcher group), deletes generated hook scripts when
they still carry the `# Generated by fallow setup-hooks.` marker, and strips the
managed block from `AGENTS.md`. Idempotent: a second run reports `unchanged` /
`not present` and exits 0.
...
`fallow init --hooks` is a different command: it scaffolds a shell-level Git
pre-commit hook under `.git/hooks/` that runs `fallow` on changed files. That is
the human enforcement path. Agent-hook setup is tool-specific and should be
treated as a reinforcement layer. Both can live in the same repo when configured
deliberately.
