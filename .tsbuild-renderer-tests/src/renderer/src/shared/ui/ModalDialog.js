 ...
 */
export function ModalDialog({ label, labelledBy, onClose, className, children }) {
    ...
    return (_jsx("dialog", { ref: dialogRef, ...(labelledBy ? { 'aria-labelledby': labelledBy } : { 'aria-label': label }), onCancel: onClose, onClose: onClose, className: cn(
        ...
        'z-50 m-auto max-h-[90vh] w-full max-w-[620px] overflow-y-auto rounded-xl border border-border-light bg-bg-secondary text-text-primary shadow-2xl backdrop:bg-black/55', className), children: children }));
}
