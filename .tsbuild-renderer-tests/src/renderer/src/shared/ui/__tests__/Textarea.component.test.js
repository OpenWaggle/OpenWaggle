...
vi.mock('@/shared/lib/shiki/highlighter', () => ({
    ...
    resolveLanguage: vi.fn((language) => (language === 'json' ? 'json' : undefined)),
}));
...
describe('Textarea', () => {
    ...
    it('renders a Shiki token overlay when a highlight language is provided', async () => {
        ...
        const { container } = render(_jsx(Textarea, { highlightLanguage: "json", value: '{\n  "mcpServers"\n}', readOnly: true }));
        await waitFor(() => {
    ...
    it('uses matching mono text metrics for highlighted code and the textarea caret layer', async () => {
        ...
        const { container } = render(_jsx(Textarea, { highlightLanguage: "json", variant: "mono", value: "{}", readOnly: true }));
        await waitFor(() => {
    ...
    it('logs and falls back when the syntax highlighter cannot load', async () => {
        ...
        const { container } = render(_jsx(Textarea, { highlightLanguage: "json", value: "{}", readOnly: true }));
        await waitFor(() => {
