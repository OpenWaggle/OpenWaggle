 ...
 */
const SELECT_BASE_CLASS = 'border bg-bg-secondary text-text-secondary outline-none transition-[border-color,box-shadow] focus:border-accent/50 focus:shadow-[0_0_0_2px_color-mix(in_srgb,var(--color-accent)_18%,transparent)]';
...
export function Select({ ref, selectSize = 'sm', className, children, ...props }) {
    return (_jsx("select", { ref: ref, className: cn(SELECT_BASE_CLASS, SELECT_SIZE_CLASS[selectSize], className), ...props, children: children }));
}
