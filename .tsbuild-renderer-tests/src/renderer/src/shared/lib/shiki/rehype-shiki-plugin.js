...
function extractLanguageFromClass(properties) {
 ...
 */
export function createRehypeShikiPlugin(options) {
    ...
    return function rehypeShikiAttacher() {
        return function rehypeShikiTransformer(tree) {
...
function visitPreElements(children, highlighter, cache) {
...
function processPreElement(pre, highlighter, cache) {
    const codeNode = pre.children.find((c) => isElement(c) && c.tagName === 'code');
    if (!codeNode)
        return;
...
function preserveLanguageClass(codeNode) {
    const lang = extractLanguageFromClass(codeNode.properties);
    if (lang)
        return { className: [`language-${lang}`] };
    return {};
}
/**
 ...
 */
export function applyShikiToHast(tree, options) {
    const { highlighter, cache } = options;
    if (highlighter === undefined)
        return;
    visitPreElements(tree.children, highlighter, cache);
...
function highlightCode(highlighter, code, language) {
    try {
        ...
        const preEl = root.children.find((c) => isElement(c) && c.tagName === 'pre');
        if (!preEl)
            return undefined;
        const codeEl = preEl.children.find((c) => isElement(c) && c.tagName === 'code');
        return codeEl;
    }
    catch (error) {
