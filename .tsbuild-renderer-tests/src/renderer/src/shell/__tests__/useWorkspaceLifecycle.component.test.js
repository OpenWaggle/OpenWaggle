...
const lifecycleMocks = vi.hoisted(() => {
    ...
    return {
        ...
        onSessionTitleUpdated: vi.fn((handler) => {
...
function runHotkey(hotkey) {
    ...
    if (!binding)
        throw new Error(`Expected hotkey ${hotkey}`);
    binding.callback();
}
describe('useWorkspaceLifecycle', () => {
    ...
    it('loads app data, subscribes to title updates, refreshes project state, and registers hotkeys', async () => {
        ...
        if (!titleHandler)
            throw new Error('Expected title subscription');
        titleHandler({ sessionId: SessionId('session-1'), title: 'New title' });
        expect(lifecycleMocks.updateSessionTitle).toHaveBeenCalledWith(SessionId('session-1'), 'New title');
        act(() => runHotkey('Mod+J'));
        act(() => runHotkey('Mod+B'));
        act(() => runHotkey('Mod+D'));
        act(() => runHotkey('Mod+K'));
