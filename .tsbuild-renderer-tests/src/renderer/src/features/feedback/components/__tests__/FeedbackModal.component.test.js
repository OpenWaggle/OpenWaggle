...
vi.mock('@/shared/lib/ipc', () => ({
    api: {
        checkGhCli: (...args) => mockCheckGhCli(...args),
        collectDiagnostics: (...args) => mockCollectDiagnostics(...args),
        submitFeedback: (...args) => mockSubmitFeedback(...args),
        generateFeedbackMarkdown: (...args) => mockGenerateFeedbackMarkdown(...args),
...
vi.mock('@/features/settings/state/preferences-store', () => ({
    usePreferencesStore: (selector) => selector({ settings: { selectedModel: 'claude-sonnet-4-20250514' } }),
}));
...
describe('FeedbackModal', () => {
    ...
    it('pre-fills from error context', () => {
        ...
        if (!(titleInput instanceof HTMLInputElement)) {
            throw new Error('Expected feedback title input');
        }
