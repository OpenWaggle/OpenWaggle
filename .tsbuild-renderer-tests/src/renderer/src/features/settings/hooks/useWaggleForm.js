...
import { useDeleteWagglePresetMutation, useSaveWagglePresetMutation, wagglePresetsQueryOptions, } from '@/queries/waggle-presets';
import { buildWaggleConfig, configMatchesPreset, INITIAL_WAGGLE_FORM_STATE, INITIAL_WAGGLE_PRESET_STATE, waggleFormReducer, wagglePresetReducer, } from './waggle-form-state';
const SLICE_ARG_2 = 60;
function describeWaggleError(error, fallback) {
    return error instanceof Error && error.message.trim() ? error.message : fallback;
}
const EMPTY_PRESETS = [];
export function useWaggleForm() {
