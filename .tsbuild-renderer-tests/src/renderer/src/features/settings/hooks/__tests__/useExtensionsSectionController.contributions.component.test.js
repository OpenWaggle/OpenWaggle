...
const OTHER_PROJECT_REGISTRY = {
    projectPaths: [OTHER_PROJECT_PATH],
    entries: [],
};
const CONTRIBUTION_REGISTRY = {
    projectPaths: [PROJECT_PATH],
...
describe('useExtensionsSectionController contribution teardown', () => {
    ...
    it('exposes non-empty contribution summaries', async () => {
        ...
        const { result } = renderHookWithQueryClient(() => useExtensionsSectionController([PROJECT_PATH]));
        await waitFor(() => {
    ...
    it('refreshes and invalidates contribution registries after disabling an extension', async () => {
        apiMock.listExtensionContributions.mockResolvedValueOnce(CONTRIBUTION_REGISTRY);
        const otherProjectContributionsKey = ['extensionContributions', OTHER_PROJECT_PATH];
        const { result, client } = renderHookWithQueryClient(() => useExtensionsSectionController([PROJECT_PATH]));
    ...
    it('applies an approved remove workflow and refreshes contribution registry', async () => {
        ...
        expect(apiMock.showConfirm).toHaveBeenCalledWith('Remove Sample Extension?', expect.stringContaining('deletes the extension package'));
