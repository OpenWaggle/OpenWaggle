...
import { describeExtensionControllerError, logMutationFailure, packageScopeToMutationScope, } from './extensions-section-controller-model';
const REMOVE_WORKFLOW_ACTOR = {
    kind: 'user',
    userId: 'settings',
};
export async function runApprovedExtensionRemoveWorkflow(input) {
    ...
    try {
        ...
        const confirmed = await api.showConfirm(`Remove ${title}?`, proposal.requiresGlobalConfirmation
        ...
        const globalConfirmation = proposal.requiresGlobalConfirmation
            ? {
                globalConfirmation: {
                    ...
                    risk: proposal.globalConfirmationRisk ??
