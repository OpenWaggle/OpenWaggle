...
const { getMcpSettingsMock, setMcpAdapterEnabledMock, setMcpServerEnabledMock, writeMcpSourceConfigMock, } = vi.hoisted(() => ({
...
describe('McpSection', () => {
    ...
    it('shows invalid source diagnostics returned by the main process', async () => {
        ...
        expect(screen.getAllByText('Invalid MCP JSON config at /Users/test/.config/mcp/mcp.json')).toHaveLength(2);
        expect(screen.getByText('Invalid Pi settings JSON at /Users/test/.pi/settings.json')).toBeInTheDocument();
    });
});
