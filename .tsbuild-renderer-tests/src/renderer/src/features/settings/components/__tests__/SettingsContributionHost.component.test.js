...
import { SettingsContributionHost, SettingsContributionSlot, SettingsContributionSlotBoundary, } from '../sections/SettingsContributionHost';
...
describe('SettingsContributionHost', () => {
    ...
    it('mounts host-renderer settings section contributions through isolated federated frames', () => {
        render(_jsx(SettingsContributionHost, { registry: registryWith([COMMAND_ENTRY, BASE_ENTRY]) }));
        ...
        expect(within(host).getByTitle('Extension module: Sample settings')).toHaveAttribute('sandbox', 'allow-scripts');
    ...
    it('returns no host when the registry has no settings section contributions', () => {
        render(_jsx(SettingsContributionHost, { registry: registryWith([COMMAND_ENTRY]) }));
    ...
    it('mounts frame execution contributions through the isolated federated frame host', async () => {
        render(_jsx(SettingsContributionHost, { registry: registryWith([FRAME_ENTRY]) }));
        const host = screen.getByLabelText('Extension settings contributions');
    ...
    it('surfaces contribution eligibility and diagnostics in the settings slot', () => {
        render(_jsx(SettingsContributionHost, { registry: registryWith([BLOCKED_ENTRY]) }));
        const host = screen.getByLabelText('Extension settings contributions');
    ...
    it('contains a settings slot failure without hiding sibling settings contributions', () => {
        ...
        render(_jsxs("div", { children: [_jsx(SettingsContributionSlotBoundary, { entry: BLOCKED_ENTRY, children: _jsx(ThrowingContribution, {}) }), _jsx(SettingsContributionSlotBoundary, { entry: BASE_ENTRY, children: _jsx(SettingsContributionSlot, { entry: BASE_ENTRY }) })] }));
        expect(screen.getByRole('alert')).toHaveTextContent('Extension settings: Blocked settings panel error');
        expect(screen.getByText('Slot exploded')).toBeInTheDocument();
        expect(screen.getByText('Sample settings')).toBeInTheDocument();
        expect(loggerMock.error).toHaveBeenCalledWith('Panel "Extension settings: Blocked settings" error', expect.objectContaining({ message: 'Slot exploded' }));
