...
const { getAppVersionMock, getUpdateStatusMock, onUpdateStatusMock, checkForUpdatesMock, installUpdateMock, } = vi.hoisted(() => ({
...
describe('GeneralSection', () => {
    beforeEach(() => {
        ...
        getUpdateStatusMock.mockResolvedValue({ type: 'idle' });
    ...
    it('shows "Check now" button when status is not-available', async () => {
        getUpdateStatusMock.mockResolvedValue({ type: 'not-available' });
        render(_jsx(GeneralSection, {}));
        await waitFor(() => {
            expect(screen.getByRole('button', { name: /check now/i })).toBeInTheDocument();
        });
    ...
    it('hides action buttons while checking or downloading', async () => {
        getUpdateStatusMock.mockResolvedValue({ type: 'checking' });
        render(_jsx(GeneralSection, {}));
        await waitFor(() => {
            expect(screen.queryByRole('button', { name: /check now/i })).not.toBeInTheDocument();
    ...
    it('calls the unsubscribe function returned by onUpdateStatus on unmount', () => {
        ...
        const { unmount } = render(_jsx(GeneralSection, {}));
        unmount();
