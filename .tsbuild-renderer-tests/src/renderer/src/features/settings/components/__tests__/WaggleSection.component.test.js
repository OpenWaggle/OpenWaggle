...
const { listWagglePresetsMock, saveWagglePresetMock, deleteWagglePresetMock, usePreferencesMock, useProvidersMock, } = vi.hoisted(() => ({
...
vi.mock('@/features/providers/components', async () => {
    ...
    return {
        ModelSelector: ({ value, onChange, providerModels, }) => (_jsx(Select, { "aria-label": "Model", value: value, onChange: (event) => onChange(event.target.value), children: providerModels.flatMap((group) => group.models.map((model) => (_jsx("option", { value: model.id, children: model.name }, model.id)))) })),
    };
});
...
describe('WaggleSection', () => {
    ...
    it('loads a selected preset into the editable form', async () => {
        ...
        renderWithQueryClient(_jsx(WaggleSection, {}));
    ...
    it('saves edits back to the active preset using the current form state', async () => {
        ...
        await waitFor(() => {
            expect(saveWagglePresetMock).toHaveBeenCalledWith(expect.objectContaining({
                id: preset.id,
                name: 'Refiner + Implementer',
                description: 'Custom: Tightens the remediation plan.',
    ...
    it('shares the Waggle preset query cache with the command palette', async () => {
        const preset = createPreset();
        listWagglePresetsMock.mockResolvedValueOnce([preset]);
        renderWithQueryClient(_jsxs(_Fragment, { children: [_jsx(WaggleSection, {}), _jsx(CommandPalette, { slashSkills: [], onSelectSkill: vi.fn(), onStartWaggle: vi.fn() })] }));
        await waitFor(() => {
