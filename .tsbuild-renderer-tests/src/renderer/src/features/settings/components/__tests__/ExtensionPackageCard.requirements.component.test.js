...
const PRIVILEGED_PACKAGE = {
    ...
    manifestPath: '/tmp/project/.openwaggle/extensions/privileged-extension/openwaggle.extension.json',
    manifest: {
        id: 'privileged-extension',
...
describe('ExtensionPackageCard requirements', () => {
    it('shows privileged consent details before trust can be granted', () => {
        render(_jsx(ExtensionPackageCard, { extensionPackage: PRIVILEGED_PACKAGE, contributionSummary: null, busy: false, projectLabel: (projectPath) => projectPath, actions: {
                onSetTrusted: vi.fn(),
                onSetEnabled: vi.fn(),
        ...
        expect(screen.getByText('Capability openwaggle.storage; methods get, set; scopes project')).toBeInTheDocument();
        expect(screen.getByText('Network access')).toBeInTheDocument();
        expect(screen.getByText('Origins https://api.github.com; access restricted')).toBeInTheDocument();
