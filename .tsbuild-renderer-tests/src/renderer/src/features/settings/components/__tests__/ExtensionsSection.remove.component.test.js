...
const { listExtensionPackagesMock, listExtensionContributionsMock, proposeExtensionPackageRemoveMock, applyExtensionPackageRemoveMock, showConfirmMock, } = vi.hoisted(() => ({
...
describe('ExtensionsSection remove workflow', () => {
    ...
    it('removes an extension package through the approved host workflow', async () => {
        ...
        await waitFor(() => {
            ...
            expect(showConfirmMock).toHaveBeenCalledWith('Remove Sample Extension?', expect.stringContaining('clears trust and enablement pins'));
