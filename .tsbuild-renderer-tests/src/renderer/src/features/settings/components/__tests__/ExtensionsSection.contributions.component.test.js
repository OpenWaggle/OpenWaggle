...
const { listExtensionPackagesMock, listContributionsMock, registerExtensionFrameMock, unregisterExtensionFrameMock, } = vi.hoisted(() => ({
...
describe('ExtensionsSection contribution registry', () => {
    beforeEach(() => {
        ...
        registerExtensionFrameMock.mockImplementation((input) => Promise.resolve({
    ...
    it('surfaces contribution registry diagnostics when no entries registered', async () => {
        ...
        expect(within(diagnostics).getByText(': Extension contribution registry build failed.')).toBeInTheDocument();
    });
});
