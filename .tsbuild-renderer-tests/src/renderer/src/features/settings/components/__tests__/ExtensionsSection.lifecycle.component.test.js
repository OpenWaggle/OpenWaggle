...
const { listExtensionPackagesMock, listExtensionContributionsMock, setExtensionTrustedMock, setExtensionEnabledMock, setExtensionProjectDisabledMock, reloadExtensionMock, } = vi.hoisted(() => ({
...
const RELOADED_VIEW = viewWithPackage(packageWithLifecycle({
    ...TRUSTED_LIFECYCLE,
    enabled: true,
...
describe('ExtensionsSection lifecycle controls', () => {
    ...
    it('trusts a discovered extension package from settings', async () => {
        ...
        renderWithQueryClient(_jsx(ExtensionsSection, {}));
    ...
    it('disables an extension only for the selected project', async () => {
        ...
        renderWithQueryClient(_jsx(ExtensionsSection, {}));
        fireEvent.click(await screen.findByRole('button', { name: 'Disable for project Sample Extension' }));
        await waitFor(() => {
        ...
        expect(screen.getByRole('button', { name: 'Enable for project Sample Extension' })).toBeInTheDocument();
    });
});
