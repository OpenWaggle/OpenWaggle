...
const { deleteSessionMock, listArchivedSessionsMock, listArchivedSessionBranchesMock, loadSessionsMock, restoreSessionBranchMock, showConfirmMock, unarchiveSessionMock, } = vi.hoisted(() => ({
...
vi.mock('@/features/sessions/state/session-store', () => ({
    useSessionStore: (selector) => selector({ loadSessions: loadSessionsMock }),
}));
function createArchivedSession(overrides) {
    return {
...
function createArchivedBranchSession(overrides) {
...
describe('ArchivedSection', () => {
    ...
    it('shows a loading state while archived sessions are being fetched', () => {
        ...
        renderWithQueryClient(_jsx(ArchivedSection, {}));
