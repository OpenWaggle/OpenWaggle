...
// ─── IPC Mock ───────────────────────────────────────────────
// The auth store imports `api` from `@/shared/lib/ipc` and calls multiple IPC methods.
...
describe('auth-store', () => {
    ...
    describe('getOAuthStatus', () => {
        ...
        it('returns the stored status when available', () => {
            const status = { type: 'in-progress', provider: 'openai' };
    ...
    describe('startOAuth', () => {
        ...
        it('updates status from onOAuthStatus listener for events without provider field', async () => {
            ...
            let resolveOAuth = () => { };
            apiMock.startOAuth.mockImplementationOnce(() => new Promise((resolve) => {
            ...
            const statusEvent = { type: 'awaiting-code' };
        ...
        it('ignores onOAuthStatus events that have a provider field', async () => {
            ...
            const statusEvent = { type: 'success', provider: 'openai' };
            for (const listener of oauthListeners) {
                listener(statusEvent);
            }
    ...
    describe('loadAllAuthAccounts', () => {
        ...
        it('succeeds even when some providers fail', async () => {
            ...
            // Should not throw
            await expect(useAuthStore
                .getState()
                .loadAllAuthAccounts(['openai-codex', 'github-copilot', 'google-gemini-cli'])).resolves.toBeUndefined();
