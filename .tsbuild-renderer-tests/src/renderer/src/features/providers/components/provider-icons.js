...
function createSvgIcon(Svg) {
    return function LobeProviderIcon({ className, style }) {
        return _jsx(Svg, { className: className, style: style, "aria-hidden": "true" });
    };
}
function FallbackProviderIcon({ className, style }) {
    return (_jsx("span", { "aria-hidden": "true", className: className, style: {
            ...style,
            borderRadius: '9999px',
            backgroundColor: typeof style?.color === 'string' && style.color !== 'currentColor'
                ? style.color
                : 'currentColor',
