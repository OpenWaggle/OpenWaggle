...
import { createOptionalCommandPaletteAction, insertCompactCommand, insertComposerCommandText, } from '../lib/command-palette-actions';
import { createBaseCommands, createConfigureWaggleItem, createPresetItems, createSkillItems, filterBaseCommands, } from '../lib/command-palette-items';
import { normalizeCommandQuery } from '../lib/command-palette-text';
import { createExtensionCommandItems, createExtensionSidePanelItems, createExtensionSlashCommandItems, resolveExtensionCommandInvocationScope, } from '../lib/extension-command-items';
...
function sessionIdFromPathname(pathname) {
...
export function useCommandPaletteItems({ query, slashSkills, onSelectSkill, onStartWaggle, onOpenSessionTree, onForkToNewSession, onCloneToNewSession, }) {
    ...
    const { data: extensionContributions = null } = useQuery(extensionContributionsQueryOptions(projectPaths, { sessionId }));
    ...
    const invokeExtensionCommand = ({ entry }) => {
    ...
    const insertExtensionSlashCommand = ({ entry }) => {
        insertComposerCommandText(extensionSlashCommandText(entry));
        closeCommandPalette();
    };
    const openExtensionSidePanel = ({ entry }) => {
        const target = {
    ...
    return [
        ...
        ...createExtensionCommandItems({
            ...
            canInvokeCommand: (entry) => resolveExtensionCommandInvocationScope({ entry, projectPath, sessionId }) !== null,
        }),
