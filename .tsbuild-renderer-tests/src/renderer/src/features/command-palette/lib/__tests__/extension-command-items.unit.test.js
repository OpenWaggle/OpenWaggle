...
import { createExtensionCommandItems, createExtensionSidePanelItems, createExtensionSlashCommandItems, resolveExtensionCommandInvocationScope, } from '../extension-command-items';
...
describe('createExtensionCommandItems', () => {
    it('maps executable command contributions to command palette items', () => {
        ...
        items[0]?.action();
        expect(invokeCommand).toHaveBeenCalledWith({ entry });
    });
