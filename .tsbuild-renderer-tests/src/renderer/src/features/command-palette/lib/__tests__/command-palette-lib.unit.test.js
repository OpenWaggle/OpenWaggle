...
const { closeCommandPaletteMock, compactCommandTextMock, focusEditorMock, getComposerStateMock, getUiStateMock, openFeedbackModalMock, setCursorIndexMock, setEditorTextMock, setInputMock, } = vi.hoisted(() => ({
...
const { createOptionalCommandPaletteAction, insertCompactCommand, openFeedbackModal } = await import('../command-palette-actions');
function item(id, section) {
    return {
        id,
        label: id,
        icon: id,
