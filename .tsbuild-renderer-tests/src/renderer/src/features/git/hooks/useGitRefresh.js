 ...
 */
export function useGitRefresh({ workingPath, repositoryPath, activeSessionId, refreshGitStatus, refreshGitBranches, refreshSession, }) {
    ...
    useEffect(() => {
        return api.onGitWorkingTreeChanged(({ workingPath: changedPath }) => {
            if (changedPath !== workingPath && changedPath !== repositoryPath)
                return;
            if (changedPath === workingPath)
                void refreshGitStatus(workingPath);
            if (changedPath === repositoryPath)
