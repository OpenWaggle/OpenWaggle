...
describe('SessionContextRow', () => {
    it('renders nothing when not visible', () => {
        const { container } = render(_jsx(SessionContextRow, { strip: stripState({ visible: false }) }));
    ...
    it('shows the environment mode and nothing that names a ref', () => {
        render(_jsx(SessionContextRow, { strip: stripState({ baseRef: 'develop' }) }));
    ...
    it('surfaces the send-block reason', () => {
        render(_jsx(SessionContextRow, { strip: stripState({ baseRef: null, sendPlan: { kind: 'blocked', reason: 'Pick a base' } }) }));
    ...
    it('offers recreate and use-checkout when the worktree has vanished', () => {
        ...
        render(_jsx(SessionContextRow, { strip: stripState({
                recreateWorktree,
                switchToLocalMode,
