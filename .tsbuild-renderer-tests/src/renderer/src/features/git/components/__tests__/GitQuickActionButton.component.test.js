...
describe('GitQuickActionButton', () => {
    it('renders the resolved label and dispatches the stacked action', () => {
        const cb = callbacks();
        render(_jsx(GitQuickActionButton, { status: status(), isBusy: false, ...cb }));
    ...
    it('dispatches pull when behind upstream', () => {
        const cb = callbacks();
        render(_jsx(GitQuickActionButton, { status: status({ hasWorkingTreeChanges: false, behindCount: 2 }), isBusy: false, ...cb }));
        fireEvent.click(screen.getByRole('button', { name: 'Pull' }));
        expect(cb.onPull).toHaveBeenCalled();
    });
    it('opens the change request when a PR is open and the ref is clean', () => {
        const cb = callbacks();
        render(_jsx(GitQuickActionButton, { status: status({
    ...
    it('is disabled while busy', () => {
        const cb = callbacks();
        render(_jsx(GitQuickActionButton, { status: status(), isBusy: true, ...cb }));
