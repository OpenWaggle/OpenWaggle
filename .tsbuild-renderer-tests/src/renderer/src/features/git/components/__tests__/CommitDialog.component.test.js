...
describe('CommitDialog', () => {
    it('commits the selected files with a trimmed message and closes on success', async () => {
        ...
        const onClose = vi.fn();
        render(_jsx(CommitDialog, { projectPath: "/repo", status: status(), statusError: null, isRefreshing: false, isCommitting: false, onRefresh: vi.fn(), onCommit: onCommit, onClose: onClose }));
        ...
        await waitFor(() => expect(onCommit).toHaveBeenCalledWith('Refactor shell', true, ['src/app.ts']));
