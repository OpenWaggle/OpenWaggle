...
import { GIT_STORE_RESET_STATE, makeBranchList, makeGitStatus, PROJECT_PATH, statusFor, } from './git-store.test-utils';
/** Status slice for one working tree, via the store's own selector. */
function sliceFor(workingPath) {
...
describe('useGitStore status and branch refresh behavior', () => {
    ...
    describe('refreshStatus', () => {
        ...
        it('sets isLoading true then false on successful fetch', async () => {
            const states = [];
            const unsubscribe = useGitStore.subscribe((state) => states.push(selectWorkingTreeStatus(state, PROJECT_PATH).isLoading));
