...
describe('resolveQuickAction', () => {
    ...
    it('offers commit & push on a dirty default ref', () => {
        expect(resolveQuickAction(status({ hasWorkingTreeChanges: true, isDefaultRef: true }), false)).toMatchObject({ action: 'commit_push' });
    });
    it('offers commit & push on a dirty branch with an open PR', () => {
        expect(resolveQuickAction(status({ hasWorkingTreeChanges: true, changeRequest: pr('open') }), false)).toMatchObject({ action: 'commit_push' });
    });
    ...
    it('offers publish repository when there is no primary remote or upstream', () => {
        expect(resolveQuickAction(status({ hasUpstream: false, hasPrimaryRemote: false }), false)).toMatchObject({ kind: 'open_publish' });
    });
    ...
    it('uses MR terminology for gitlab', () => {
        const result = resolveQuickAction(status({ hasWorkingTreeChanges: true, sourceControlProvider: provider('gitlab') }), false);
        expect(result.label).toContain('MR');
    });
    it('prefers push (not create PR) when clean, ahead, and a PR is open', () => {
        expect(resolveQuickAction(status({ aheadCount: 2, changeRequest: pr('open') }), false)).toMatchObject({
            action: 'push',
        });
    });
    ...
    it('falls back to plain commit when dirty with no primary remote', () => {
        expect(resolveQuickAction(status({ hasWorkingTreeChanges: true, hasPrimaryRemote: false, hasUpstream: false }), false)).toMatchObject({ action: 'commit' });
    });
    ...
    it('uses push-only (commit_push) on the default ref when no upstream and ahead', () => {
        expect(resolveQuickAction(status({ hasUpstream: false, aheadCount: 1, isDefaultRef: true }), false)).toMatchObject({ action: 'commit_push' });
    });
});
function pr(state) {
