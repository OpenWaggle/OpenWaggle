...
function renderDialogSurface(input) {
    return render(_jsx(ExtensionDialogSurfaceContent, { actions: {
            onClose: input.onClose ?? vi.fn(),
            onRefresh: vi.fn(),
        }, error: input.error ?? null, loading: input.loading ?? false, projectPaths: [PROJECT_PATH], registry: input.registry, surfacePayload: { surface: 'dialog-test' }, target: {
