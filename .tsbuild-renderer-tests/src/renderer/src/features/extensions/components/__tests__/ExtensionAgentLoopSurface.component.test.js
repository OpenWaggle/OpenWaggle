...
import { CUSTOM_INTERACTION_UNAVAILABLE_ACTION_ID, ExtensionAgentLoopSurface, } from '../ExtensionAgentLoopSurface';
...
describe('ExtensionAgentLoopSurface', () => {
    ...
    it('renders an OpenWaggle-owned tool fallback when no renderer matches', () => {
        render(_jsx(ExtensionAgentLoopSurface, { input: { surface: 'tool', toolCall: { ...TOOL_PART, name: 'missing.tool' } }, projectPaths: [PROJECT_PATH], registry: REGISTRY }));
    ...
    it('mounts matching renderers and passes serializable surface payload to the frame document', async () => {
        render(_jsx(ExtensionAgentLoopSurface, { input: { surface: 'tool', toolCall: TOOL_PART }, projectPaths: [PROJECT_PATH], registry: REGISTRY }));
    ...
    it('renders an interactive fallback and routes selected actions', () => {
        ...
        render(_jsx(ExtensionAgentLoopSurface, { input: {
                surface: 'interaction',
                interaction: {
                    id: 'select-issue',
                ...
                onAction,
            }, projectPaths: [PROJECT_PATH], registry: { projectPaths: [PROJECT_PATH], entries: [] } }));
    ...
    it('renders an explicit custom interaction failure when no desktop renderer matches', () => {
        ...
        expect(screen.getByText(/does not execute Pi TUI custom components inside Electron/)).toBeInTheDocument();
        ...
        expect(onAction).toHaveBeenCalledWith('custom-interaction', CUSTOM_INTERACTION_UNAVAILABLE_ACTION_ID);
    });
    it('mounts matching custom interaction renderer contributions', async () => {
        ...
        render(_jsx(ExtensionAgentLoopSurface, { input: {
            ...
            }, projectPaths: [PROJECT_PATH], registry: { projectPaths: [PROJECT_PATH], entries: [interactionEntry] } }));
        ...
        expect(screen.queryByText('Custom desktop interaction renderer unavailable')).not.toBeInTheDocument();
    });
    it('mounts transcript renderer contributions for transcript surfaces', async () => {
        ...
        render(_jsx(ExtensionAgentLoopSurface, { fallback: null, input: {
                surface: 'transcript',
                transcript: {
                    sessionId: 'session-1',
                    projectPaths: [PROJECT_PATH],
            ...
            }, projectPaths: [PROJECT_PATH], registry: { projectPaths: [PROJECT_PATH], entries: [transcriptEntry] } }));
