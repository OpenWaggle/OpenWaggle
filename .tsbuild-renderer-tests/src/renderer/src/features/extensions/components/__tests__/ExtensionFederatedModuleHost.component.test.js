...
function dispatchFrameMessage(frame, message) {
...
describe('ExtensionFederatedModuleHost', () => {
    ...
    it('routes frame surface actions to the host without invoking broker IPC', async () => {
        ...
        render(_jsx(ExtensionFederatedModuleHost, { entry: ENTRY, onSurfaceAction: onSurfaceAction }));
        const frame = extensionFrame();
        dispatchFrameMessage(frame, {
            type: 'surface-action',
    ...
    it('does not remount when an equivalent surface payload is recreated', async () => {
        const { rerender } = render(_jsx(ExtensionFederatedModuleHost, { entry: ENTRY, surfacePayload: {
                surface: 'status',
        ...
        rerender(_jsx(ExtensionFederatedModuleHost, { entry: ENTRY, surfacePayload: {
                surface: 'status',
                status: { label: 'Thinking', tone: 'running' },
            } }));
    ...
    it('mounts frame execution through the same isolated federated module contract', async () => {
        render(_jsx(ExtensionFederatedModuleHost, { entry: { ...ENTRY, execution: OPENWAGGLE_EXTENSION.EXECUTION_PLACEMENT.FRAME } }));
        const frame = extensionFrame();
        expect(frame).toHaveAttribute('sandbox', EXTENSION_FEDERATED_MODULE_IFRAME_SANDBOX);
        expect(screen.getByText(/Mounting extension module/)).toBeInTheDocument();
        await waitFor(() => {
