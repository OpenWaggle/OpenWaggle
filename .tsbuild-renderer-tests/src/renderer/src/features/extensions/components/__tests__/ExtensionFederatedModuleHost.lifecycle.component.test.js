...
function dispatchResizeFromWindow(frameId, height) {
...
describe('ExtensionFederatedModuleHost lifecycle performance', () => {
    ...
    it('does not remount when an equivalent registry entry object is recreated', async () => {
        const { rerender } = render(_jsx(ExtensionFederatedModuleHost, { entry: equivalentEntry() }));
        ...
        rerender(_jsx(ExtensionFederatedModuleHost, { entry: equivalentEntry() }));
