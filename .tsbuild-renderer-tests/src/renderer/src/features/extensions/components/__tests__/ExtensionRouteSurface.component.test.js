...
function renderRouteSurface(input) {
    return render(_jsx(ExtensionRouteSurfaceContent, { error: input.error ?? null, extensionId: "sample-extension", loading: input.loading ?? false, onRefresh: vi.fn(), projectPaths: [PROJECT_PATH], registry: input.registry, routeId: input.routeId ?? 'sample.route' }));
}
