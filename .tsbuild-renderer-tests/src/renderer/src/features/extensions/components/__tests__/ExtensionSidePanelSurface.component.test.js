...
function renderSidePanelSurface(input) {
    return render(_jsx(ExtensionSidePanelSurfaceContent, { error: input.error ?? null, loading: input.loading ?? false, onClose: input.onClose ?? vi.fn(), onRefresh: vi.fn(), projectPaths: [PROJECT_PATH], registry: input.registry, target: {
            extensionId: 'sample-extension',
...
describe('ExtensionSidePanelSurfaceContent', () => {
    ...
    it('disposes and removes the mounted frame when the registry entry disappears', async () => {
        ...
        rerender(_jsx(ExtensionSidePanelSurfaceContent, { error: null, loading: false, onClose: vi.fn(), onRefresh: vi.fn(), projectPaths: [PROJECT_PATH], registry: EMPTY_REGISTRY, target: {
                extensionId: 'sample-extension',
