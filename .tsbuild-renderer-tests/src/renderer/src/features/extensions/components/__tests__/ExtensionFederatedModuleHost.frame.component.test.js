...
function extensionFrameHost(frame) {
...
function dispatchOpenExternal(frame, url) {
...
function dispatchResize(frame, height) {
    const frameWindow = stableExtensionFrameWindow(frame);
    window.dispatchEvent(new MessageEvent('message', {
        source: frameWindow,
        data: {
...
function dispatchInvoke(frame, requestId) {
    const frameWindow = stableExtensionFrameWindow(frame);
    window.dispatchEvent(new MessageEvent('message', {
        source: frameWindow,
        data: {
...
describe('ExtensionFederatedModuleHost frame shell', () => {
    ...
    it('can mount as a bare full-height surface when the container owns the chrome', async () => {
        render(_jsx(ExtensionFederatedModuleHost, { chrome: "bare", entry: ENTRY, fill: true }));
    ...
    it('asks the isolated frame to dispose on unmount', async () => {
        const { unmount } = render(_jsx(ExtensionFederatedModuleHost, { entry: ENTRY }));
        const frame = extensionFrame();
    ...
    it('registers the protocol frame and configures it after the ready handshake', async () => {
        render(_jsx(ExtensionFederatedModuleHost, { entry: ENTRY }));
        const frame = extensionFrame();
        const postMessage = vi.spyOn(stableExtensionFrameWindow(frame), 'postMessage');
        await waitFor(() => {
    ...
    it('auto-sizes non-fill frames from sandbox resize messages', async () => {
        render(_jsx(ExtensionFederatedModuleHost, { autoHeight: true, entry: ENTRY, maxAutoHeight: 300, minAutoHeight: 100 }));
        const frame = extensionFrame();
    ...
    it('opens frame-requested external links through the host IPC bridge', async () => {
        ...
        await waitFor(() => {
            expect(apiMock.openExternal).toHaveBeenCalledWith('https://github.com/OpenWaggle/OpenWaggle/issues/113');
        });
    });
