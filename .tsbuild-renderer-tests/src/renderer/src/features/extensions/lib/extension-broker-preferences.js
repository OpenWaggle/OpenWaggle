...
function extensionInvokeResultMutatesPreferences(result) {
    if (!result.ok) {
        return false;
    }
...
function extensionInvokeResultMutatesRuntimeContributions(result) {
    if (!result.ok) {
        return false;
    }
    ...
    return (result.value.method === OPENWAGGLE_EXTENSION_BROKER.METHOD.REGISTER_CONTRIBUTION ||
        result.value.method === OPENWAGGLE_EXTENSION_BROKER.METHOD.UNREGISTER_CONTRIBUTION);
}
export async function refreshPreferencesAfterExtensionInvoke(result) {
    const refreshes = [];
