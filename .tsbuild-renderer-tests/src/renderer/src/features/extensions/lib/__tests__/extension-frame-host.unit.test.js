...
import { decodeExtensionFrameMessage, extensionFrameConfig, extensionInvokeInputFromFrame, postFrameMessage, } from '../extension-frame-host';
const ENTRY = {
...
describe('extension frame host helpers', () => {
    it('creates typed frame execution mount configuration without DOM or SDK fields', () => {
        const config = extensionFrameConfig({
            entry: ENTRY,
            moduleUrl: 'openwaggle-extension://runtime/module/%2Ftmp%2Fproject%2F.openwaggle%2Fextensions%2Fsample-extension/abcdef/%5B%22%2Ftmp%2Fproject%22%5D/dist/settings.js',
        });
    ...
    it('decodes only extension frame messages for the mounted frame id', () => {
        ...
        expect(decodeExtensionFrameMessage({ ...message, channel: 'other-channel' }, 'frame-1')).toBeNull();
    });
