...
import { resolveExtensionAgentLoopContribution, resolveExtensionAgentLoopContributionEntries, } from '../extension-agent-loop-resolution';
...
describe('resolveExtensionAgentLoopContribution', () => {
    ...
    it('matches custom-message and interaction renderers by their specific match lists', () => {
        ...
        expect(resolveExtensionAgentLoopContribution({
    ...
    it('resolves auxiliary placement contributions that bind to the same Pi tool event', () => {
        ...
        const target = { surface: 'tool', toolName: 'sample.tool' };
        expect(resolveExtensionAgentLoopContributionEntries({
            registry: registryView,
            target,
            requestedProjectPaths: [PROJECT_PATH],
            family: OPENWAGGLE_EXTENSION.CONTRIBUTION_FAMILY.DIALOGS,
        }).map((contribution) => contribution.entry)).toEqual([dialogEntry]);
        expect(resolveExtensionAgentLoopContributionEntries({
            registry: registryView,
            target,
            requestedProjectPaths: [PROJECT_PATH],
            family: OPENWAGGLE_EXTENSION.CONTRIBUTION_FAMILY.SIDE_PANELS,
        }).map((contribution) => contribution.entry)).toEqual([sidePanelEntry]);
        expect(resolveExtensionAgentLoopContributionEntries({
            registry: registryView,
            target,
            requestedProjectPaths: [PROJECT_PATH],
            family: OPENWAGGLE_EXTENSION.CONTRIBUTION_FAMILY.STATUS_WIDGETS,
        }).map((contribution) => contribution.entry)).toEqual([statusEntry]);
    });
