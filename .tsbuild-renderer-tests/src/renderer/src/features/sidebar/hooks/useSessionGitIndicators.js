 ...
 */
export function useSessionGitIndicators(sessions) {
    ...
    useEffect(() => {
        ...
        return api.onGitWorkingTreeChanged(({ workingPath }) => {
            if (tracked.has(workingPath))
                void refreshStatus(workingPath);
        });
    }, [workingPathKey, refreshStatus]);
}
/** The working tree a session row describes, or null when it has no project. */
export function sessionWorkingPath(session) {
    return resolveSessionWorkingDir({ environmentMode: session.environmentMode, worktreePath: session.worktreePath ?? null }, session.projectPath);
}
/** One session's working-tree indicator, empty until its status is known. */
export function useSessionGitIndicator(session) {
...
function distinctWorkingPaths(sessions) {
    const paths = new Set();
