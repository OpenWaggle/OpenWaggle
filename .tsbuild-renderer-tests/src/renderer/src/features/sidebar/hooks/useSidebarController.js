...
function removeCollapsedProject(current, path) {
    if (!current.has(path))
        return current;
...
function toggleCollapsedProject(current, path) {
    const next = new Set(current);
    if (next.has(path))
        next.delete(path);
    else
...
function createDomainActions(state, clearTransientDraftContext, refreshGit, refreshAfterSessionMutation) {
...
function buildControllerOutput(state, actions, clearTransientDraftContext) {
    return {
        ...state,
        ...
        handleToggleProjectCollapsed(path) {
...
export function useSidebarController() {
    ...
    const actions = createDomainActions(state, clearTransientDraftContext, refreshGit, refreshAfterSessionMutation);
