...
const { archiveSessionMock, cancelAgentMock, deleteSessionMock, getGitStatusMock, getProjectPreferencesMock, getProviderModelsMock, listActiveRunsMock, listArchivedSessionsMock, listGitBranchesMock, navigateMock, openPathMock, routerState, showConfirmMock, updateSettingsMock, } = vi.hoisted(() => ({
...
vi.mock('@tanstack/react-router', () => ({
    ...
    useRouterState: (options) => options.select({ location: { pathname: routerState.pathname } }),
}));
...
describe('Sidebar project actions', () => {
    ...
    it('hides collapsed sidebar contents from accessibility and hit testing', () => {
        ...
        const { container } = render(_jsx(Sidebar, {}));
    ...
    it('archives all visible project sessions with a count-aware confirmation', async () => {
        ...
        await waitFor(() => {
            expect(showConfirmMock).toHaveBeenCalledWith(expect.stringContaining('Archive 1 session'), expect.stringContaining(PROJECT_PATH));
    ...
    it('permanently removes all project sessions and project references', async () => {
        ...
        await waitFor(() => {
            expect(showConfirmMock).toHaveBeenCalledWith(expect.stringContaining('permanently delete 2 sessions'), expect.stringContaining(PROJECT_PATH));
