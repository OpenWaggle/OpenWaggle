...
function renderItem(target) {
    return render(_jsx("ul", { children: _jsx(SessionListItem, { session: target, isActive: false, actions: actions() }) }));
}
describe('SessionListItem git indicator', () => {
    ...
    it('gives two sessions on two worktrees independent indicators', () => {
        ...
        const first = renderItem(session('a', { environmentMode: 'worktree', worktreePath: WORKTREE_A }));
