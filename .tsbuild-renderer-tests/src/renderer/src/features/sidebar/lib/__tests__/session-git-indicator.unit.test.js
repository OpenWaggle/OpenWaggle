...
describe('buildSessionGitIndicator', () => {
    ...
    it('uses the singular for one changed file', () => {
        expect(buildSessionGitIndicator(status({ clean: false, filesChanged: 1 })).description).toBe('1 changed file');
    });
    ...
    it('combines dirtiness with divergence', () => {
        const indicator = buildSessionGitIndicator(status({ clean: false, filesChanged: 2, ahead: 1, behind: 3 }));
