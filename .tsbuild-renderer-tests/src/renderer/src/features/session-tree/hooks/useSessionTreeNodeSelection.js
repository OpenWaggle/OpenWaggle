...
import { createBranchDraftSelectionFromNode, maybeOpenBranchSummaryPrompt, setComposerTextValue, } from '@/features/chat/lib';
...
export function useSessionTreeNodeSelection(input) {
    ...
    function selectMaterializedBranch(node, branchId) {
    ...
    function selectDraftBranch(node) {
        const tree = input.tree;
        if (!tree) {
            return;
        }
