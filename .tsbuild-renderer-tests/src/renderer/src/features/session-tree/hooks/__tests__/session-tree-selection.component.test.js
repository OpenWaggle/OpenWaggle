...
describe('useSessionTreeNodeSelection', () => {
    ...
    it('navigates materialized branch nodes through Pi and refreshes the selected workspace', async () => {
        ...
        const { result } = renderHook(() => useSessionTreeNodeSelection({
            activeWorkspace: workspace(),
        ...
        expect(selectionMocks.navigateSessionTree).toHaveBeenCalledWith(SESSION_ID, SupportedModelId('openai/gpt-5.5'), ASSISTANT_NODE.id, { summarize: false });
        await waitFor(() => expect(refreshSessionWorkspace).toHaveBeenCalledWith(SESSION_ID, {
