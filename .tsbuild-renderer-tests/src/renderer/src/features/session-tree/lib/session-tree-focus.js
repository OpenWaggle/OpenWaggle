...
export function findFirstVisibleChildIndex(visibleRows, currentIndex) {
...
export function findVisibleParentIndex(visibleRows, currentIndex) {
    const currentRow = visibleRows[currentIndex];
...
export function clampSessionTreeFocusIndex(currentIndex, visibleCount) {
...
export function moveSessionTreeFocus({ currentIndex, visibleCount, direction, }) {
    if (visibleCount <= SESSION_TREE.TRAVERSAL.FIRST_INDEX) {
        return SESSION_TREE.TRAVERSAL.FIRST_INDEX;
    }
