...
function appendGutterDepth(gutterDepths, nextDepth) {
    if (gutterDepths.includes(nextDepth)) {
        return gutterDepths;
    }
    return [...gutterDepths, nextDepth];
}
function orderedChildren(children, activePathIds, originalOrderById) {
    return [...children].sort((left, right) => {
        ...
        return ((originalOrderById.get(sessionTreeNodeKey(left)) ?? SESSION_TREE.TRAVERSAL.FIRST_INDEX) -
            (originalOrderById.get(sessionTreeNodeKey(right)) ?? SESSION_TREE.TRAVERSAL.FIRST_INDEX));
    });
}
function isHiddenByCollapsedAncestor(node, nodeById, expandedNodeIdSet) {
...
function findNearestVisibleParentId(node, nodeById, visibleNodeIdSet) {
    let currentParentId = sessionTreeParentKey(node);
    while (currentParentId) {
        const parent = nodeById.get(currentParentId);
        if (!parent) {
...
function appendChild(childrenByParentId, parentId, child) {
...
function buildChildrenByVisibleParent(input) {
    ...
    for (const node of input.nodes) {
        appendChild(childrenByParentId, findNearestVisibleParentId(node, input.nodeById, visibleNodeIdSet), node);
    }
...
function getOrderedChildren(context, childrenByParentId, parentId) {
    return orderedChildren(childrenByParentId.get(parentId ? String(parentId) : null) ?? [], context.activePathIds, context.originalOrderById);
}
...
function pushRows(context, input) {
    const displayedChildren = getOrderedChildren(context, context.displayedChildrenByParentId, input.node.id);
    const expandableChildren = getOrderedChildren(context, context.expandableChildrenByParentId, input.node.id);
    const hasNextSibling = input.siblingIndex < input.siblingCount - SESSION_TREE.TRAVERSAL.NEXT_ITEM_DELTA;
...
function buildRowBuilderContext(input) {
    ...
    const displayedNodes = input.filteredNodes.filter((node) => !isHiddenByCollapsedAncestor(node, nodeById, expandedNodeIdSet));
    return {
...
export function getVisibleSessionTreeRows(input) {
...
export function getVisibleSessionTreeNodes(nodes, expandedNodeIds) {
