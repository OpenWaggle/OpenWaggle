...
import { getVisibleSessionTreeRows } from '../session-tree-visibility';
...
export function rowDepths(rows) {
    return rows.map((row) => row.visualDepth);
}
export function connectorLineXs(lines) {
    return lines.map((line) => line.xPx);
}
export function rowAt(rows, index) {
...
export function nodeAt(nodes, index) {
