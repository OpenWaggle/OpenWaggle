...
import { clampSessionTreeFocusIndex, findFirstVisibleChildIndex, findVisibleParentIndex, moveSessionTreeFocus, } from '../session-tree-visibility';
import { FIFTH_ROW_INDEX, FIRST_ROW_INDEX, node, SECOND_ROW_INDEX, TREE, visibleRows, } from './session-tree-test-fixtures';
...
describe('moveSessionTreeFocus', () => {
    it('keeps keyboard focus inside the visible row bounds', () => {
        expect(moveSessionTreeFocus({ currentIndex: 0, visibleCount: 3, direction: 'previous' })).toBe(0);
        expect(moveSessionTreeFocus({ currentIndex: 2, visibleCount: 3, direction: 'next' })).toBe(2);
        expect(moveSessionTreeFocus({ currentIndex: 1, visibleCount: 3, direction: 'previous' })).toBe(0);
