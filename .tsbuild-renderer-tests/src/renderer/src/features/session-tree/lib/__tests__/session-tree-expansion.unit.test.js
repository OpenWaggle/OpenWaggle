...
import { getDefaultExpandedSessionTreeNodeIds, resolveExpandedSessionTreeNodeIds, resolveSessionTreeRowExpandedNodeIds, } from '../session-tree-visibility';
...
describe('getDefaultExpandedSessionTreeNodeIds', () => {
    it('expands every parent node by default', () => {
        expect(getDefaultExpandedSessionTreeNodeIds(TREE).map((expandedNodeId) => String(expandedNodeId))).toEqual(['root', 'child-a']);
    });
});
