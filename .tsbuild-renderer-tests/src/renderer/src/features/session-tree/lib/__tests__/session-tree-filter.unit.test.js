...
describe('searchSessionTreeNodes', () => {
    it('searches visible node text with normalized whitespace and casing', () => {
        ...
        expect(searchSessionTreeNodes({ nodes, filteredNodes: nodes, query: '  BRANCH   animation ' }).map((item) => String(item.id))).toEqual(['root', 'answer']);
    });
    it('preserves visible ancestors for matching descendants', () => {
        ...
        expect(searchSessionTreeNodes({
            nodes,
            filteredNodes: [nodes[0], nodes[2]],
            query: 'needle',
        }).map((item) => String(item.id))).toEqual(['root', 'leaf']);
    });
    it('searches persisted content when a node has no hydrated message', () => {
        ...
        expect(searchSessionTreeNodes({ nodes, filteredNodes: nodes, query: 'branch summary text' }).map((item) => String(item.id))).toEqual(['summary-node']);
    });
    it('searches branch ids so visible branch badges can be found', () => {
        ...
        expect(searchSessionTreeNodes({ nodes, filteredNodes: nodes, query: 'search-branch' }).map((item) => String(item.id))).toEqual(['head']);
    });
