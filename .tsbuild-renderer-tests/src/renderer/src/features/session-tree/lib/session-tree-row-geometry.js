...
function nodeStemBottom(row, nodeCenter) {
    const linearSiblingContinuation = row.hasNextSibling &&
...
function parentStemBottom(row, nodeCenter, parentCenter) {
...
function branchElbow(nodeCenter, parentCenter) {
...
export function getSessionTreeRowGeometry(row) {
    ...
    const ancestorLines = row.gutterDepths.flatMap((depth) => depth === row.visualDepth
        ? []
        : [
            verticalConnector({
