...
import { node, SESSION_ID, treeUiState, visibleRows, } from '../../lib/__tests__/session-tree-test-fixtures';
...
describe('Session Tree panel components', () => {
    it('renders header controls and delegates close', () => {
        const onClose = vi.fn();
        render(_jsx(SessionTreePanelHeader, { onClose: onClose }));
    ...
    it('renders filter controls and delegates search/filter changes', () => {
        ...
        render(_jsx(SessionTreePanelFilters, { filters: {
                filterMode: 'default',
                searchQuery: '',
                onFilterModeChange,
                onSearchQueryChange,
    ...
    it('renders empty tree states without row actions', () => {
        const { rerender } = render(_jsx(SessionTreePanelContent, { content: makeContent({}) }));
        expect(screen.getByText('No session tree yet.')).toBeInTheDocument();
        rerender(_jsx(SessionTreePanelContent, { content: makeContent({
                tree: makeTree(),
                searchActive: true,
    ...
    it('renders visible rows, records row refs, and delegates row actions', () => {
        ...
        render(_jsx(SessionTreeRows, { actions: { focusIndex, selectNode, toggleNodeExpanded }, refs: { rowRefs }, view: {
