...
function renderPanel(previewMarkdown) {
    mockState.previewMarkdown = previewMarkdown;
    return render(_jsx(SkillsPanel, {}));
}
describe('SkillsPanel markdown safety', () => {
    ...
    it('renders allowed links and blocks unsafe protocols', () => {
        renderPanel('[good](https://example.com) [email](mailto:test@example.com) [bad](javascript:alert(1))');
        expect(screen.getByRole('link', { name: 'good' })).toHaveAttribute('href', 'https://example.com');
        expect(screen.getByRole('link', { name: 'email' })).toHaveAttribute('href', 'mailto:test@example.com');
