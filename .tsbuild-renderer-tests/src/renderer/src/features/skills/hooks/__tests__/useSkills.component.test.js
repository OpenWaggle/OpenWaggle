...
const { getSkillPreviewMock, getStandardsStatusMock, listSkillsMock, setSkillEnabledMock } = vi.hoisted(() => ({
...
function createCatalog(skills) {
    return {
        projectPath: '/tmp/project',
        skills,
    };
}
function skillAt(catalog, index) {
...
describe('useSkills', () => {
    ...
    it('loads skills resources and preview for the selected skill', async () => {
        ...
        listSkillsMock.mockResolvedValueOnce(createCatalog([
            {
                id: 'skill-one',
                name: 'Skill One',
