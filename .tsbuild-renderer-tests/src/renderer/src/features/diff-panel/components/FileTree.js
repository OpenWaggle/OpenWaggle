...
import { buildNavigatorTree, NAVIGATOR_ROOT_ID, } from '@/features/diff-panel/lib/navigator-tree';
 ...
 */
function isClickHandler(value) {
    return typeof value === 'function';
}
const ROOT_NODE = { path: NAVIGATOR_ROOT_ID, name: 'Changed files', isFile: false };
function FileChangeBadges({ stats }) {
    return (_jsxs("span", { className: "ml-auto flex shrink-0 items-center gap-1 pl-1", children: [stats.additions > 0 ? (_jsxs("span", { className: "text-[10px] text-diff-add-mark", children: ["+", String(stats.additions)] })) : null, stats.deletions > 0 ? (_jsxs("span", { className: "text-[10px] text-diff-remove-text", children: ["-", String(stats.deletions)] })) : null, _jsx("span", { role: "img", "aria-label": stats.status, title: stats.status, className: cn('w-2 text-center text-[10px] font-semibold', STATUS_CLASS[stats.status]), children: STATUS_GLYPH[stats.status] })] }));
}
/**
 ...
 */
function useNavigatorTree(files) {
    ...
    const expandedItems = useMemo(() => folderIds.filter((id) => !collapsed.has(id)), [folderIds, collapsed]);
    return useTree({
        ...
        setState: (updater) => {
            ...
            if (next.expandedItems === undefined)
                return;
            ...
            setCollapsed((prev) => {
                ...
                if (computed.length === prev.size && computed.every((id) => prev.has(id)))
                    return prev;
...
export function FileTree({ files, onFileClick }) {
    ...
    return (_jsxs("div", { 
        ...
        className: "relative flex h-full max-w-[45%] shrink-0 flex-col border-l border-border bg-diff-bg py-2", style: { width: `${String(width)}px` }, children: [_jsx(Button, { variant: "unstyled", type: "button", "aria-label": `Resize changed file list, currently ${String(width)} pixels`, title: "Drag or use arrow keys to resize", onPointerDown: (event) => {
                    ...
                    if (event.key === 'ArrowRight') {
                ...
                }, className: cn('absolute inset-y-0 left-0 z-10 w-1 cursor-col-resize border-0 bg-transparent p-0 transition-colors', isResizing ? 'bg-accent/60' : 'hover:bg-accent/40 focus-visible:bg-accent/60') }), _jsxs("div", { ...tree.getContainerProps(), className: "flex-1 overflow-auto outline-none", children: [_jsx(AssistiveTreeDescription, { tree: tree }), tree.getItems().map((item) => {
                        const data = item.getItemData();
                        if (data.path === NAVIGATOR_ROOT_ID)
                            return null;
                        ...
                        return (_createElement(Button, { variant: "unstyled", type: "button", ...itemProps, key: item.getId(), onClick: (event) => {
                            ...
                            }, className: cn('flex h-[22px] w-full items-center gap-1.5 pr-1.5 text-left outline-none', item.isFocused() && 'bg-bg-hover', item.isSelected() && 'bg-diff-highlight-bg', 'hover:bg-bg-hover') },
                            isFolder ? (_jsx(ChevIcon, { className: "size-[11px] shrink-0 text-text-tertiary" })) : (_jsx("span", { className: "size-[11px] shrink-0" })),
                            _jsx("span", { className: cn('truncate text-[12px]', data.isFile ? 'text-text-primary' : 'text-text-secondary'), children: data.name }),
                            data.stats ? _jsx(FileChangeBadges, { stats: data.stats }) : null));
                    })] })] }));
}
