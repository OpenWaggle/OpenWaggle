...
import { buildBaseRefChoices } from '@/features/diff-panel/lib/base-ref-choices';
...
export function useBaseRefChoices(projectPath) {
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                if (!cancelled)
                    setChoices(buildBaseRefChoices(result.branches));
            }
            catch (error) {
