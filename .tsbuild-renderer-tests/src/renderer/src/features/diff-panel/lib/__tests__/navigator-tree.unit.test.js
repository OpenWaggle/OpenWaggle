...
describe('navigator tree', () => {
    ...
    it('derives status from the patch mode header, not the line counts', () => {
        expect(fileChangeStats(file('a.ts', 'diff --git a/a.ts b/a.ts\nnew file mode 100644\n@@ -0,0 +1 @@\n+x'))).toMatchObject({ status: 'added' });
        expect(fileChangeStats(file('b.ts', 'diff --git a/b.ts b/b.ts\ndeleted file mode 100644\n@@ -1 +0,0 @@\n-x'))).toMatchObject({ status: 'deleted' });
