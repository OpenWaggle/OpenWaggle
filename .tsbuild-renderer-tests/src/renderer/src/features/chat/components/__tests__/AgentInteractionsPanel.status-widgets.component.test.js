...
function statusWidgetRegistry(kind) {
    const entry = {
...
describe('AgentInteractionsPanel status widgets', () => {
    ...
    it('mounts status widgets that target the same pending Pi interaction', async () => {
        ...
        render(_jsx(AgentInteractionsPanel, { interactions: [interaction], extensionRegistry: statusWidgetRegistry('confirm'), extensionProjectPaths: [PROJECT_PATH], onRespond: vi.fn().mockResolvedValue(undefined) }));
