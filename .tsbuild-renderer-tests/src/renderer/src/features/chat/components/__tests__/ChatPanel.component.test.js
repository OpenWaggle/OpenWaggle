...
const useChatPanelSectionsMock = vi.hoisted(() => vi.fn());
...
function createSections(overrides = {}, composerOverrides = {}) {
    const transcript = {
        messages: [],
        isLoading: false,
        projectPath,
...
function renderPanel(overrides = {}, composerOverrides = {}) {
    ...
    return render(_jsx(QueryClientProvider, { client: queryClient, children: _jsx(ChatPanel, {}) }));
}
describe('ChatPanel', () => {
    beforeEach(() => {
    ...
    it('routes custom branch-summary submission through send instead of enqueue while loading', () => {
        ...
        expect(onSendWithWaggle).toHaveBeenCalledWith(expect.objectContaining({ text: 'focus on decisions' }));
