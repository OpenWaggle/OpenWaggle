...
vi.mock('../StreamingText', () => ({
    StreamingText: ({ text }) => (_jsx("div", { "data-testid": "summary-markdown", children: text })),
}));
...
describe('CompactionSummaryCard', () => {
    it('renders collapsed by default and expands the generated summary inline', () => {
        render(_jsx(CompactionSummaryCard, { summary: '## Goal\nKeep the failing compaction test context.', tokensBefore: 123456 }));
        ...
        expect(screen.getByTestId('summary-markdown')).toHaveTextContent('Keep the failing compaction test context.');
    });
});
