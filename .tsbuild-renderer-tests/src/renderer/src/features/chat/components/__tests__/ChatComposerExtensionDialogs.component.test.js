...
function extensionDialogEntry({ kind, title = 'GitHub approval dialog', scope = {
    kind: OPENWAGGLE_EXTENSION.SCOPE.PROJECT_KIND,
    label: 'Project',
    projectPath: PROJECT_PATH,
}, packagePath = `${PROJECT_PATH}/.openwaggle/extensions/github-fixture`, manifestPath = `${PROJECT_PATH}/.openwaggle/extensions/github-fixture/openwaggle.extension.json`, contentHash = 'abcdef', trusted = true, }) {
    return {
...
function extensionDialogRegistry(kind) {
...
function extensionFrameWindow(frame) {
...
function stableExtensionFrameWindow(frame) {
...
function extensionFrameId(frame) {
...
function dispatchSurfaceAction(frame, actionId, payload) {
...
describe('ChatComposerExtensionDialogs', () => {
    beforeEach(() => {
        ...
        apiMock.registerExtensionFrame.mockImplementation((input) => Promise.resolve({
    ...
    it('opens matching extension dialog contributions from compact composer launchers', () => {
        render(_jsx(ChatComposerExtensionDialogs, { agentInteractions: [pendingInteraction()], extensionProjectPaths: [PROJECT_PATH], extensionRegistry: extensionDialogRegistry('confirm'), onRespond: noOpRespond() }));
    ...
    it('does not render composer launchers without matching dialog contributions', () => {
        render(_jsx(ChatComposerExtensionDialogs, { agentInteractions: [pendingInteraction()], extensionProjectPaths: [PROJECT_PATH], extensionRegistry: extensionDialogRegistry('select'), onRespond: noOpRespond() }));
    ...
    it('opens the selected package when global and project dialogs share an id', () => {
        render(_jsx(ChatComposerExtensionDialogs, { agentInteractions: [pendingInteraction()], extensionProjectPaths: [PROJECT_PATH], extensionRegistry: {
                projectPaths: [PROJECT_PATH],
                entries: [
    ...
    it('submits pending Pi interaction responses from composer-launched extension dialogs', async () => {
        ...
        render(_jsx(ChatComposerExtensionDialogs, { agentInteractions: [interaction], extensionProjectPaths: [PROJECT_PATH], extensionRegistry: extensionDialogRegistry('confirm'), onRespond: onRespond }));
        fireEvent.click(screen.getByRole('button', { name: /extensions/i }));
        fireEvent.click(screen.getByRole('button', { name: /github approval dialog/i }));
        ...
        await waitFor(() => {
            expect(screen.queryByRole('dialog', { name: 'GitHub approval dialog' })).not.toBeInTheDocument();
        });
    });
    it('ignores duplicate surface responses while the interaction response is pending', async () => {
        const interaction = pendingInteraction();
        let resolveResponse;
        const onRespond = vi.fn(() => new Promise((resolve) => {
