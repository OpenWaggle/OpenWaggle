...
const apiMock = vi.hoisted(() => ({
    registerExtensionFrame: vi.fn((input) => Promise.resolve({
...
vi.mock('../ChatRowRenderer', () => ({
    ChatRowRenderer: ({ row }) => (_jsx("div", { children: row.type === 'message'
            ? row.message.parts.reduce((text, part) => (part.type === 'text' ? `${text}${part.content}` : text), '')
            : 'row-content' })),
}));
vi.mock('../WelcomeScreen', () => ({
    WelcomeScreen: () => _jsx("div", { children: "welcome" }),
}));
...
function createMessageChatRow(message) {
    return {
        type: 'message',
        message,
        isStreaming: false,
...
function configureScrollableElement(scroller) {
...
describe('ChatTranscript t3-style scroll behavior', () => {
    beforeEach(() => {
        ...
        vi.useFakeTimers();
        vi.spyOn(window, 'requestAnimationFrame').mockImplementation((callback) => window.setTimeout(() => callback(performance.now()), REQUEST_ANIMATION_FRAME_DELAY_MS));
    ...
    it('keeps a single non-overflowing row at the top', async () => {
        const { container } = render(_jsx(ChatTranscript, { section: createSection({
                lastUserMessageId: 'msg-1',
    ...
    it('follows streaming text growth when the message count stays stable', async () => {
        ...
        const { container, rerender } = render(_jsx(ChatTranscript, { section: createSection({
                isLoading: true,
        ...
        rerender(_jsx(ChatTranscript, { section: createSection({
                isLoading: true,
