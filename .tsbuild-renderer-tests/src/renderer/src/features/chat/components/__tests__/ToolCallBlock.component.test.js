...
describe('ToolCallBlock', () => {
    ...
    it('renders natural action text for known tool', () => {
        render(_jsx(ToolCallBlock, { name: "read", args: '{"path":"src/main.ts"}', state: "input-complete", result: { content: 'file content', state: 'success' } }));
        expect(screen.getByText('Read src/main.ts')).toBeInTheDocument();
    });
    it('renders tool name as fallback for unknown tool', () => {
        render(_jsx(ToolCallBlock, { name: "customTool", args: "{}", state: "input-complete", result: { content: '', state: 'success' } }));
        expect(screen.getByText('customTool')).toBeInTheDocument();
    });
    it('shows running action text with ellipsis when running', () => {
        render(_jsx(ToolCallBlock, { name: "read", args: '{"path":"src/index.ts"}', state: "running", isStreaming: true }));
        expect(screen.getByText('Reading src/index.ts...')).toBeInTheDocument();
    });
    it('shows spinner when tool is running', () => {
        const { container } = render(_jsx(ToolCallBlock, { name: "bash", args: '{"command":"ls"}', state: "running", isStreaming: true }));
        const spinner = container.querySelector('.animate-spin');
        expect(spinner).toBeTruthy();
    });
    it('does not render running state for historical unresolved tool calls', () => {
        const { container } = render(_jsx(ToolCallBlock, { name: "write", args: '{"path":"out.txt"}', state: "input-complete" }));
    ...
    it('shows check icon when completed successfully', () => {
        const { container } = render(_jsx(ToolCallBlock, { name: "read", args: '{"path":"file.ts"}', state: "input-complete", result: { content: 'file content', state: 'success' } }));
    ...
    it('shows error text when result has error', () => {
        render(_jsx(ToolCallBlock, { name: "read", args: '{"path":"missing.ts"}', state: "input-complete", result: { content: '', state: 'error', error: 'File not found' } }));
        expect(screen.getByText('Failed read missing.ts')).toBeInTheDocument();
    });
    it('renders completed state once a concrete result exists', () => {
        render(_jsx(ToolCallBlock, { name: "write", args: '{"path":"docs/SUMMARY.md"}', state: "input-complete", result: {
    ...
    it('shows bash with backtick-wrapped verb', () => {
        render(_jsx(ToolCallBlock, { name: "bash", args: '{"command":"pnpm test"}', state: "input-complete", result: { content: '"passed"', state: 'success' } }));
    ...
    it('shows result content when expanded and completed', () => {
        render(_jsx(ToolCallBlock, { name: "read", args: '{"path":"file.ts"}', state: "input-complete", result: { content: '"hello world"', state: 'success' } }));
        fireEvent.click(screen.getByRole('button'));
        expect(screen.getByText('Result')).toBeInTheDocument();
    });
    it('shows command with $ prefix for bash args when expanded', () => {
        render(_jsx(ToolCallBlock, { name: "bash", args: '{"command":"npm test"}', state: "input-complete", result: { content: '"passed"', state: 'success' } }));
        fireEvent.click(screen.getByRole('button'));
        expect(screen.getByText('$')).toBeInTheDocument();
    });
    it('shows error content when expanded and errored', () => {
        render(_jsx(ToolCallBlock, { name: "read", args: '{"path":"missing.ts"}', state: "input-complete", result: { content: '', state: 'error', error: 'ENOENT' } }));
        fireEvent.click(screen.getByRole('button'));
        expect(screen.getByText('ENOENT')).toBeInTheDocument();
    });
    it('handles malformed JSON args gracefully', () => {
        render(_jsx(ToolCallBlock, { name: "read", args: "not-json", state: "input-complete" }));
        fireEvent.click(screen.getByRole('button'));
        expect(screen.getByText('not-json')).toBeInTheDocument();
    });
    it('renders Pi structured text results without exposing raw JSON', () => {
        render(_jsx(ToolCallBlock, { name: "bash", args: '{"command":"pnpm test"}', state: "complete", result: {
                content: {
    ...
    it('renders Pi edit diff details inline for small diffs', () => {
        render(_jsx(ToolCallBlock, { name: "edit", args: '{"path":"src/app.ts"}', state: "complete", result: {
                content: {
    ...
    it('syntax highlights read tool file content using the existing Shiki pipeline', async () => {
        const { container } = render(_jsx(ToolCallBlock, { name: "read", args: '{"path":"src/example.ts"}', state: "complete", result: { content: 'const value = 1', state: 'complete' } }));
    ...
    it('uses plain rendering for very large file previews to keep expansion responsive', () => {
        ...
        const { container } = render(_jsx(ToolCallBlock, { name: "read", args: '{"path":"src/example.ts"}', state: "complete", result: { content: largeContent, state: 'complete' } }));
        fireEvent.click(screen.getByRole('button', { name: /Read src\/example\.ts/ }));
        expect(screen.getByText('Large file preview shown without syntax highlighting to keep the UI responsive.')).toBeInTheDocument();
        expect(container.querySelector('code.language-typescript')).toBeNull();
    });
    it('copies path values from expanded tool details', () => {
        render(_jsx(ToolCallBlock, { name: "read", args: '{"path":"src/main.ts"}', state: "complete", result: { content: 'file content', state: 'complete' } }));
