...
function pendingConfirmInteraction() {
    return {
...
function interactionRequestEvent(interaction) {
    return {
...
function multiSurfaceInteractionRegistry() {
    return {
        projectPaths: [PROJECT_PATH],
        entries: [
            extensionEntry({
...
function AgentLoopExtensionSurfaceHarness({ interaction, registry, onRespond, }) {
    const extensions = { registry, projectPaths: [PROJECT_PATH] };
    return (_jsxs(_Fragment, { children: [_jsx(InteractionEventRow, { event: interactionRequestEvent(interaction), extensions: extensions }), _jsx(AgentInteractionsPanel, { extensionProjectPaths: [PROJECT_PATH], extensionRegistry: registry, interactions: [interaction], onRespond: onRespond }), _jsx(ChatComposerExtensionDialogs, { agentInteractions: [interaction], extensionProjectPaths: [PROJECT_PATH], extensionRegistry: registry, onRespond: onRespond })] }));
}
describe('agent-loop extension surfaces', () => {
    ...
    it('renders the same pending Pi interaction across transcript, live, status, and composer extension surfaces', async () => {
        ...
        render(_jsx(AgentLoopExtensionSurfaceHarness, { interaction: interaction, onRespond: vi.fn().mockResolvedValue(undefined), registry: registry }));
        ...
        await waitFor(() => {
            ...
            expect(screen.getByTitle('Extension module: GitHub status widget')).toHaveAttribute('src', expect.stringContaining(EXTENSION_FRAME_URL_PREFIX));
        });
