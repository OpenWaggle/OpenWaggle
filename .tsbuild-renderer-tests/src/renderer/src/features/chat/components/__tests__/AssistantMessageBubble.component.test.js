...
vi.mock('../StreamingText', () => ({
    StreamingText: ({ text }) => _jsx("div", { "data-testid": "streaming-text", children: text }),
}));
vi.mock('../ToolCallRouter', () => ({
    ToolCallRouter: ({ part }) => (_jsx("div", { "data-testid": "tool-call-router", children: part.name })),
}));
vi.mock('../AgentLabel', () => ({
    AgentLabel: ({ assistantModel, waggle, }) => {
        if (!assistantModel && !waggle)
            return null;
        return (_jsxs("div", { "data-testid": "agent-label", children: [waggle?.agentLabel, assistantModel] }));
    },
}));
vi.mock('../CollapsibleDetails', () => ({
    CollapsibleDetails: ({ collapseLabel, }) => _jsx("div", { "data-testid": "collapsible-details", children: collapseLabel }),
}));
// ---------------------------------------------------------------------------
...
function toolCallPart(name, id = 'tc-1') {
    return { type: 'tool-call', id, name, arguments: '{}', state: 'output-available' };
}
function toolResultPart(toolCallId) {
    return {
        type: 'tool-result',
        toolCallId,
        content: 'ok',
...
function createMessage(id, parts) {
    return { id, role: 'assistant', parts };
}
const defaultSessionId = SessionId('session-1');
function renderAssistantMessage({ message, waggle, assistantModel, hideAgentLabel, actions, }) {
    return render(_jsx(AssistantMessageBubble, { message: message, runtime: {
            sessionId: defaultSessionId,
            extensions: { registry: null, projectPaths: [] },
        }, run: assistantModel ? { assistantModel } : undefined, waggle: waggle, presentation: hideAgentLabel ? { hideAgentLabel } : undefined, actions: actions }));
}
function setCollapse(overrides) {
