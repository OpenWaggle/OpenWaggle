...
function registryWithInteractionRenderer(kind) {
    const entry = {
...
describe('AgentInteractionsPanel', () => {
    ...
    it('submits pending Pi confirm interactions from the fallback panel', () => {
        ...
        render(_jsx(AgentInteractionsPanel, { interactions: [interaction], onRespond: onRespond }));
    ...
    it('shows matching extension interaction renderer without blocking fallback controls', () => {
        ...
        render(_jsx(AgentInteractionsPanel, { interactions: [interaction], extensionRegistry: registryWithInteractionRenderer('confirm'), extensionProjectPaths: [projectPath], onRespond: onRespond }));
    ...
    it('shows an explicit failure for custom interactions without a desktop renderer', () => {
        ...
        render(_jsx(AgentInteractionsPanel, { interactions: [interaction], onRespond: onRespond }));
        expect(screen.getByText(`Custom interaction · ${OPENWAGGLE_AGENT_LOOP.PI_TUI_CUSTOM_INTERACTION_TYPE}`)).toBeInTheDocument();
    ...
    it('submits custom interaction responses from matching extension renderers', async () => {
        ...
        render(_jsx(AgentInteractionsPanel, { interactions: [interaction], extensionRegistry: registryWithInteractionRenderer(OPENWAGGLE_AGENT_LOOP.PI_TUI_CUSTOM_INTERACTION_TYPE), extensionProjectPaths: [projectPath], onRespond: onRespond }));
        const frame = extensionFrame();
        await waitFor(() => {
            expect(frame).toHaveAttribute('src', expect.stringContaining(EXTENSION_FRAME_URL_PREFIX));
        });
