...
describe('chat auxiliary cards', () => {
    it('renders branch summaries and delegates branch creation', () => {
        ...
        render(_jsx(BranchSummaryCard, { id: "message-1", summary: "Use a dedicated renderer feature boundary.", onBranchFromMessage: onBranchFromMessage }));
    ...
    it('renders run summaries with merged visible phases', () => {
        render(_jsx(RunSummary, { totalMs: 3_000, phases: [
    ...
    it('renders interrupted run metadata and delegates dismissal', () => {
        ...
        render(_jsx(InterruptedRunNotice, { runId: "run-1", branchId: branchId, runMode: "waggle", model: SupportedModelId('openai/gpt-5.5'), interruptedAt: interruptedAt, onDismiss: onDismiss }));
