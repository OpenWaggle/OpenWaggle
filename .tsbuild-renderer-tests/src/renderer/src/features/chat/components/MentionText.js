...
export function renderTextWithMentions(text) {
    ...
    for (const match of text.matchAll(MENTION_REGEX)) {
        ...
        key += 1;
        parts.push(_jsxs("span", { className: cn('bg-accent/10 text-accent rounded px-1.5 py-0.5 text-[13px]', 'inline-flex items-center gap-1', 'select-none cursor-default'), title: filePath, children: [_jsx(FileText, { size: ICON_SIZE, className: "shrink-0" }), _jsx("span", { children: basename })] }, key));
