...
import { CUSTOM_INTERACTION_RESPONSE_ACTION_ID, CUSTOM_INTERACTION_UNAVAILABLE_ACTION_ID, } from '@/features/extensions';
function typedResponseFromPayload(interaction, payload) {
...
export function responseFromExtensionAction(input) {
    ...
    return matchBy(input.interaction, 'kind')
        ...
        if (input.actionId === 'reject')
            return { kind: 'confirm', accepted: false };
        return null;
    })
        .with('select', (interaction) => {
        if (input.actionId === 'cancel')
            return { kind: 'select', selected: null };
        ...
        .with('editor', () => {
        if (input.actionId === 'cancel')
            return { kind: 'editor', value: null };
        return input.actionId === 'submit' && typeof input.payload === 'string'
            ? { kind: 'editor', value: input.payload }
            : null;
    })
        .with('notify', () => input.actionId === 'acknowledge' ? { kind: 'notify', acknowledged: true } : null)
        .with('custom', () => {
