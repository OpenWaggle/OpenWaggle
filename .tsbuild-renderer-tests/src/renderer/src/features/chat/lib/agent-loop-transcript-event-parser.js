...
import { baseEventFields, isObject, optionalJsonValue, parseJsonObject, stringField, } from './agent-loop-transcript-event-fields';
import { parseErrorInfo, parseInteraction, parseInteractionKind, parseInteractionStatus, } from './agent-loop-transcript-interactions';
...
function parseInteractionRequestEvent(event) {
...
function parseInteractionResolvedEvent(event) {
...
export function parseAgentLoopEvent(event) {
    if (!isObject(event)) {
        return null;
    }
...
export function isAgentLoopTranscriptNode(node) {
    if (node.kind !== 'custom') {
        return false;
    }
...
export function readAgentLoopEventFromNode(node) {
