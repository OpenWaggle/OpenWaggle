...
import { finishToolExecution, startToolExecution, updateToolExecution, } from './chat-stream-tool-events';
export function applyAgentTransportEvent(messages, event) {
    const cloneMessages = () => messages.slice();
    return matchBy(event, 'type')
        .with('agent_start', 'agent_end', 'turn_start', 'turn_end', cloneMessages)
        .with('message_start', (value) => value.role === 'assistant'
        ...
        .with('queue_update', 'compaction_start', 'compaction_end', 'auto_retry_start', 'auto_retry_end', 'agent_interaction_request', 'agent_interaction_resolved', 'custom', cloneMessages)
        .exhaustive();
}
