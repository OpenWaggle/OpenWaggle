...
export function maybeOpenBranchSummaryPrompt(input) {
    ...
    void api
        .getPiBranchSummarySkipPrompt(input.activeWorkspace?.tree.session.projectPath ?? input.projectPath)
        .then((skipPrompt) => {
        if (!skipPrompt) {
            openIfCurrent();
        }
    })
        .catch((skipPromptError) => {
        const message = skipPromptError instanceof Error ? skipPromptError.message : String(skipPromptError);
