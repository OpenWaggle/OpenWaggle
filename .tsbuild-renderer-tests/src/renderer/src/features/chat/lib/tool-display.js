...
function isPiNativeToolName(name) {
...
function getToolActionText(name, args, isRunning) {
    ...
    if (!label)
        return isRunning ? `${verb}...` : verb;
    if (isRunning && name === 'bash')
        return `${verb} ${label}`;
    return isRunning ? `${verb} ${label}...` : `${verb} ${label}`;
}
function formatToolTarget(name, args, primaryArg) {
    return match({ name, args })
        .with({ name: 'bash', args: { command: P.select('command', P.string) } }, ({ command }) => `\`${command}\``)
        .with({ name: 'read', args: { path: P.select('path', P.string) } }, ({ path }) => `${path}${formatReadLineSuffix(args)}`)
...
function formatReadLineSuffix(args) {
...
export function resolveActionText(params) {
...
function formatStatusActionText(prefix, name, args) {
