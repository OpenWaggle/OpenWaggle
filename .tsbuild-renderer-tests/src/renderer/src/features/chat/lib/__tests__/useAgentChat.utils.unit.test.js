...
import { appendMissingOptimisticUserMessages, appendUnpersistedAssistantTail, formatAttachmentPreview, mergeBackgroundReconnectMessages, sessionToUIMessages, } from '../useAgentChat.utils';
...
describe('appendUnpersistedAssistantTail', () => {
    it('preserves live assistant output after a matching refreshed user snapshot', () => {
        ...
        expect(appendUnpersistedAssistantTail(snapshotMessages, [
...
describe('mergeBackgroundReconnectMessages', () => {
    it('does not duplicate an optimistic user message already present in the reconnect snapshot', () => {
        ...
        expect(mergeBackgroundReconnectMessages([persistedUser], [optimisticUser, cachedAssistant])).toEqual([persistedUser, cachedAssistant]);
    });
});
