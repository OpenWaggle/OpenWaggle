...
import { buildFencedCodeMarkdown, buildTailPreview, getEditUnifiedDiff, getResultError, getStringArg, getToolResultText, inferLanguageFromPath, shouldHighlightCode, } from '../tool-call-block';
...
describe('tool call block view helpers', () => {
    ...
    it('extracts explicit and structured error messages', () => {
        expect(getResultError({ state: 'success', content: 'ok', error: 'explicit failure' })).toBe('explicit failure');
        expect(getResultError({ state: 'error', content: 'runtime failure' })).toBe('runtime failure');
        expect(getResultError({ state: 'success', content: { error: 'payload failure' } })).toBe('payload failure');
    ...
    it('builds a fenced code block with a fence longer than embedded backticks', () => {
        expect(buildFencedCodeMarkdown('const s = ```', 'typescript')).toBe('````typescript\nconst s = ```\n````');
    });
