...
import { getMaxScrollTop, isScrollContainerNearBottom, scrollElementToBottom, } from '../scroll-to-bottom';
describe('scroll-to-bottom helpers', () => {
    it('treats invalid positions as near-bottom to avoid blocking autoscroll', () => {
        expect(isScrollContainerNearBottom({ scrollTop: Number.NaN, clientHeight: 100, scrollHeight: 200 })).toBe(true);
    });
    it('clamps negative thresholds and detects distance from bottom', () => {
        expect(isScrollContainerNearBottom({ scrollTop: 100, clientHeight: 100, scrollHeight: 200 }, -1)).toBe(true);
        expect(isScrollContainerNearBottom({ scrollTop: 20, clientHeight: 100, scrollHeight: 200 }, 10)).toBe(false);
    });
...
describe('queue failure feedback', () => {
    it('logs and notifies when auto-send fails', () => {
        ...
        reportAutoSendQueueFailure({ logger, showToast }, SessionId('session-1'), { text: 'queued prompt', thinkingLevel: 'medium', attachments: [] }, new Error('offline'));
        expect(logger.error).toHaveBeenCalledWith('Failed to auto-send queued message', {
            sessionId: SessionId('session-1'),
            error: 'offline',
            queuedText: 'queued prompt',
        });
        expect(showToast).toHaveBeenCalledWith('Queued message failed to send automatically. It stayed in the queue.');
    });
    it('logs and notifies when queued steering fails with non-error values', () => {
        ...
        expect(showToast).toHaveBeenCalledWith('Could not steer the queued message. It was returned to the queue.');
    });
});
