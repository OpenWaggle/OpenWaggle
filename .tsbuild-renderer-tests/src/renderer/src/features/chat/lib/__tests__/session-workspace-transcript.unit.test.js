...
function workspaceWithPath(nodes, activeNodeId, lastActiveNodeId) {
    return {
        tree: {
            session: {
                id: SESSION_ID,
...
function activeNodeIdCreatedOrder(nodes, activeNodeId) {
...
describe('resolveTranscriptMessages', () => {
    it('uses the selected workspace transcript path instead of later main-branch messages', () => {
        ...
        const resolved = resolveTranscriptMessages({
            ...
            activeWorkspace: workspaceWithPath([beforeBranch, answerBeforeBranch, branchPoint, afterBranch], branchPoint.id, afterBranch.id),
            messages: [
