...
function actionText(name, args, isRunning) {
...
describe('resolveActionText', () => {
    ...
    it('includes read line ranges when offset and limit are present', () => {
        expect(actionText('read', { path: 'src/main/index.ts', offset: 10, limit: 5 }, false)).toBe('Read src/main/index.ts:10-14');
    });
    ...
    it('formats Pi filesystem tool targets', () => {
        expect(actionText('grep', { pattern: 'TODO', path: 'src', glob: '*.ts' }, false)).toBe('Searched /TODO/ in src (*.ts)');
