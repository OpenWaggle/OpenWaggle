...
import { createBranchDraftSelection, createBranchDraftSelectionFromNode, shouldPromptForBranchSummary, } from '../branch-from-message';
...
function workspace(entryNode, allNodes = [entryNode]) {
    return {
        tree: {
            session: {
                id: SESSION_ID,
...
describe('createBranchDraftSelectionFromNode', () => {
    it('mirrors Pi retry semantics for Session Tree user-message nodes', () => {
        const result = createBranchDraftSelectionFromNode(node('user-2', 'assistant-parent', { kind: 'user_message', text: 'Retry with tests' }));
        expect(result).toEqual({
            sourceNodeId: SessionNodeId('assistant-parent'),
            routeNodeId: SessionNodeId('assistant-parent'),
