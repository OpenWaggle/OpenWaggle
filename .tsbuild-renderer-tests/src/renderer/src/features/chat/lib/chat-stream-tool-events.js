import { ensureToolCall, findAssistantMessageIdForToolCall, findLatestAssistantMessageId, stringifyToolInput, updateAssistantParts, updateToolCall, } from './chat-stream-state-helpers';
export function startToolExecution(messages, event) {
...
export function updateToolExecution(messages, event) {
    const targetAssistantId = findAssistantMessageIdForToolCall(messages, event.toolCallId) ??
...
export function finishToolExecution(messages, event) {
    const targetAssistantId = findAssistantMessageIdForToolCall(messages, event.toolCallId) ??
        findLatestAssistantMessageId(messages);
    const ensuredMessages = targetAssistantId
        ? ensureToolCall(messages, targetAssistantId, event.toolCallId, event.toolName, event.args)
    ...
    return updateAssistantParts(updatedMessages, resultAssistantId, (parts) => {
        const withoutPreviousResult = parts.filter((part) => part.type !== 'tool-result' || part.toolCallId !== event.toolCallId);
        return [
