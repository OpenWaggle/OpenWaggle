...
const EMPTY_WAGGLE_METADATA_LOOKUP = Object.freeze({});
function getSessionConfig(session, activeConfig, activeCollaborationId, configSessionId) {
...
function getCurrentAgentMetadata(params) {
...
function resolveAssistantMetadata(params) {
    ...
    if (params.messageIndex > params.lastUserMessageIndex &&
...
function buildPersistedMetaMap(session) {
    const map = new Map();
    if (!session)
        return map;
 ...
 */
export function useWaggleMetadataLookup(session, messages) {
