...
import { extensionSlashCommandPayload, parseCompactCommand, parseExtensionSlashCommand, parseSessionCopyCommand, } from '@/features/composer/commands';
...
async function compactSession(params, customInstructions) {
...
function extensionSlashCommandScope(params) {
...
async function invokeExtensionSlashCommand(params, command) {
    const { entry } = command;
...
async function handleSendCommand(params, text) {
...
function activeWaggleConfigForSend(params) {
    if (!params.waggleConfig)
        return null;
    if (params.waggleStatus !== 'idle')
        return null;
    if (params.waggleOwningId && params.waggleOwningId !== params.activeSessionId)
        return null;
    return params.waggleConfig;
}
async function sendThroughActiveMode(params, payload) {
...
export function useChatSendWorkflow(params) {
    return {
        async sendWithWaggle(payload) {
            if (await handleSendCommand(params, payload.text))
                return;
            const draftBranchReady = await params.branchSummary.materializeDraftBranchForSend(params.draftBranch);
            if (!draftBranchReady)
                return;
