...
import { appendMissingOptimisticUserMessages, appendUnpersistedAssistantTail, buildPartialAssistantMessage, mergeBackgroundReconnectMessages, reconcileSnapshotUserMessages, sessionToUIMessages, } from '../lib/useAgentChat.utils';
import { buildOptimisticMessagesKey, buildSessionSnapshotKey, getMessagesForSession, mergeSessionAndOptimisticMessages, setMessagesForSession, updateMessagesForSession, } from './useAgentChat.message-cache';
function resolvePendingForegroundRun(context) {
    const pending = context.pendingRunWaiterRef.current;
    context.pendingRunWaiterRef.current = null;
    pending?.resolve();
}
function clearForegroundRunState(context) {
...
export function resetMissingSessionHydration(context) {
...
function getSessionHydrationKeys(input, context) {
...
function updateHydrationKeys(sessionId, keys, context) {
...
function resetSessionChangedState(keys, context) {
...
function shouldKeepForegroundHydration(input, keys, context) {
    if (!context.foregroundStreamActiveRef.current) {
        return false;
    }
...
function shouldSkipActiveRunHydration(input, keys, context) {
    return (context.backgroundReconnectSessionIdRef.current === input.sessionId &&
        !keys.sessionChanged &&
        !keys.snapshotChanged &&
        !keys.optimisticChanged);
}
function handleActiveRunReconnectResult(capturedSessionId, nextMessages, context) {
    if (!nextMessages ||
        context.currentSessionIdRef.current !== capturedSessionId ||
        context.backgroundReconnectSessionIdRef.current !== capturedSessionId) {
        return;
    }
    updateMessagesForSession(context.messagesBySessionIdRef, context.setMessagesBySessionId, context.setRunRenderMessages, capturedSessionId, (currentMessages) => mergeBackgroundReconnectMessages(nextMessages, currentMessages), { cacheRunSnapshot: true });
}
function handleActiveRunReconnectError(capturedSessionId, reconnectError, context) {
    if (context.currentSessionIdRef.current !== capturedSessionId) {
        return;
    }
    context.setError(reconnectError instanceof Error ? reconnectError : new Error(String(reconnectError)));
    context.setStatus('error');
    context.setBackgroundStreaming(false);
    context.backgroundStreamingRef.current = false;
}
function hydrateActiveRunSession(input, keys, context) {
    if (shouldSkipActiveRunHydration(input, keys, context)) {
        return;
    }
    const persistedMessages = mergeSessionAndOptimisticMessages(input.session, input.optimisticUserMessages);
    const nextMessages = input.cachedRenderMessages
        ...
        : reconcileSnapshotUserMessages(persistedMessages, getMessagesForSession(context.messagesBySessionIdRef, input.sessionId));
    setMessagesForSession(context.messagesBySessionIdRef, context.setMessagesBySessionId, context.setRunRenderMessages, input.sessionId, nextMessages, { cacheRunSnapshot: true });
    ...
    void reconnectToBackgroundRun(input.sessionId, input.session, input.optimisticUserMessages)
        .then((nextReconnectMessages) => handleActiveRunReconnectResult(input.sessionId, nextReconnectMessages, context))
        .catch((reconnectError) => handleActiveRunReconnectError(input.sessionId, reconnectError, context));
}
function hydrateIdleSession(input, keys, context) {
    context.setBackgroundStreaming(false);
    context.backgroundStreamingRef.current = false;
    context.backgroundReconnectSessionIdRef.current = null;
    ...
    const snapshotMessages = appendMissingOptimisticUserMessages(sessionToUIMessages(input.session), input.optimisticUserMessages);
    ...
    setMessagesForSession(context.messagesBySessionIdRef, context.setMessagesBySessionId, context.setRunRenderMessages, input.sessionId, appendUnpersistedAssistantTail(reconciledMessages, existingMessages));
    updateHydrationKeys(input.sessionId, keys, context);
    if (keys.sessionChanged) {
        context.setStatus('ready');
        context.setError(undefined);
...
export function hydrateSessionMessages(input, context) {
...
async function reconnectToBackgroundRun(sessionId, session, optimisticUserMessages) {
    const latestSession = await api.getSessionDetail(sessionId);
    const snapshot = await api.getBackgroundRun(sessionId);
    const historicalMessages = mergeSessionAndOptimisticMessages(latestSession ?? session, optimisticUserMessages);
    if (!snapshot) {
