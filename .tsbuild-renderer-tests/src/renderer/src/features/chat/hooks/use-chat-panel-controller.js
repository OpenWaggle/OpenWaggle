...
export function useChatPanelSections() {
    ...
    const { activeSessionId, activeSession, createSession, setActiveSession, refreshSession } = env.chat;
    const { activeWorkspace, clearDraftBranchForSession, commandPaletteOpen, draftBranch, handleDismissInterruptedRun, handleOpenProject, handleSelectProjectPath, loadSessions, model, navigate, openSettings, projectPath, recentProjects, refreshSessionWorkspace, setDraftBranch, showToast, thinkingLevel, } = env;
    const { messages, sendMessage, sendWaggleMessage, isLoading, status, stop, steer, error, withDeferredSnapshotRefresh, previewSteeredUserTurn, streamSignalVersion, compactionStatus, agentInteractions, agentCustomMessages, agentInteractionEvents, respondAgentInteraction, } = useAgentChat(activeSessionId, activeSession, model, thinkingLevel);
    ...
    async function handleStarterPrompt(content) {
        if (!model.trim()) {
    ...
    const extensionContributionsQuery = useQuery(extensionContributionsQueryOptions(extensionProjectPaths, { sessionId: activeSessionId }));
    ...
    const waggleStatus = waggleOwningId && waggleOwningId !== activeSessionId ? 'idle' : waggleStoreStatus;
    ...
    useAutoSendQueue({
        ...
        paused: isSteering,
        onSendFailure: (payload, sendError) => reportAutoSendQueueFailure({ logger, showToast }, activeSessionId, payload, sendError),
    });
    function handleBranchFromMessage(messageId) {
        if (!activeSessionId)
            return;
    ...
    const { turnAnchorMessageIds, handleViewTurnDiff } = useTurnReveal(activeSessionId, navigate, messages.length);
    const transcript = useTranscriptSection({
        ...
        handleForkFromMessage: (messageId) => void sessionCopy.forkMessageToNewSession(messageId),
        handleViewTurnDiff,
