...
import { apiMock, createSessionWithMessages, emitAgentEvent, emitRunCompleted, installUseAgentChatTestLifecycle, SEND_PAYLOAD, useAgentChat, } from './useAgentChat.test-utils';
const waggleConfig = {
    mode: 'sequential',
    agents: [
        {
...
function expectPartialAssistantVisible(messages) {
...
describe('useAgentChat Waggle stop', () => {
    ...
    it('keeps partial assistant output visible after a mid-turn stop snapshot refresh', async () => {
        ...
        const { result, rerender } = renderHook(({ session }) => useAgentChat(SessionId('session-1'), session, SupportedModelId('claude-sonnet-4-5'), 'medium'), { initialProps: { session: createSessionWithMessages(1, []) } });
