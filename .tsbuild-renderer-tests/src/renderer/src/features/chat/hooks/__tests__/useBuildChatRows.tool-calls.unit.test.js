...
import { createAssistantPendingToolMessage, createAssistantTerminalToolMessage, createAssistantToolMessage, createToolResultMessage, createUserMessage, getAssistantMessageRows, getWaggleTurnRows, } from './useBuildChatRows.test-utils';
describe('buildChatRows tool-call rendering', () => {
    ...
    it('renders pending and terminal repeated tool rows independently', () => {
        const messages = [
            ...
            createAssistantTerminalToolMessage('assistant-2', 'tool-a', 'The file write completed successfully.'),
        ];
        const assistantRows = getAssistantMessageRows(messages);
        expect(assistantRows).toHaveLength(2);
