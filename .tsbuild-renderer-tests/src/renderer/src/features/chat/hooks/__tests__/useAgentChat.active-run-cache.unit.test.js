...
import { apiMock, createDeferred, createSessionWithIdAndMessages, hasActiveRunMock, installUseAgentChatTestLifecycle, runRenderSnapshots, useAgentChat, } from './useAgentChat.test-utils';
describe('useAgentChat active run cache', () => {
    ...
    it('keeps active run render caches isolated across sessions with the same starter prompt', async () => {
        ...
        const createPersistedSession = (id) => createSessionWithIdAndMessages(id, 1, [
            {
        ...
        await waitFor(() => {
            expect(result.current.messages.filter((message) => getUIMessageText(message) === prompt)).toHaveLength(1);
        ...
        await waitFor(() => {
            ...
            expect(result.current.messages).not.toEqual(expect.arrayContaining([expect.objectContaining({ id: 'assistant-b' })]));
        });
    });
});
