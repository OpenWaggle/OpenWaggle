...
function agentLoopEventNode(id, parentId, event, createdOrder) {
    return {
        id: SessionNodeId(id),
        sessionId: SESSION_ID,
        parentId: parentId ? SessionNodeId(parentId) : null,
...
function agentLoopCustomRows(rows) {
    return rows.filter((row) => row.type === 'agent-loop-custom-message');
}
function agentLoopInteractionRows(rows) {
...
describe('useTranscriptSection agent-loop hydration', () => {
    ...
    it('hydrates persisted agent-loop custom and interaction events into chat rows', () => {
        ...
        const persistedCustom = agentLoopEventNode('run-1:agent-loop:0', 'assistant-previous', {
            type: 'custom',
            timestamp: 20,
            name: 'openwaggle.github.issues.summary',
            value: { open: 2 },
        }, 1);
        const persistedRequest = agentLoopEventNode('run-1:agent-loop:1', 'run-1:agent-loop:0', {
        ...
        const persistedResolved = agentLoopEventNode('run-1:agent-loop:2', 'run-1:agent-loop:1', {
