...
import { apiMock, createDeferred, createSessionWithIdAndMessages, emitAgentEvent, hasActiveRunMock, installUseAgentChatTestLifecycle, runRenderSnapshots, SEND_PAYLOAD, setRunRenderMessagesMock, useAgentChat, } from './useAgentChat.test-utils';
describe('useAgentChat session switching', () => {
    ...
    it('does not wipe an active run render snapshot with stale render output during a switch', async () => {
        ...
        const backgroundRun = createDeferred();
        hasActiveRunMock.mockImplementation((id) => id === sessionB);
        ...
        const { result, rerender } = renderHook(({ sessionId, session, }) => useAgentChat(sessionId, session, SupportedModelId('claude-sonnet-4-5'), 'medium'), {
            initialProps: {
                sessionId: sessionA,
        ...
        const sessionBSnapshotWrites = setRunRenderMessagesMock.mock.calls.filter(([id]) => id === sessionB);
    ...
    it('does not cache an active session transcript under another active session during a switch', async () => {
        const sendA = createDeferred();
        const sendB = createDeferred();
        ...
        expect(runRenderSnapshots.get('session-b')?.messages).not.toEqual(expect.arrayContaining([expect.objectContaining({ id: 'assistant-a' })]));
        expect(result.current.messages).not.toEqual(expect.arrayContaining([expect.objectContaining({ id: 'assistant-a' })]));
