...
import { createSession, createSessionWithMessages, installUseAgentChatTestLifecycle, useAgentChat, } from './useAgentChat.test-utils';
describe('useAgentChat hydration', () => {
    ...
    it('rehydrates the same session when a newer persisted snapshot arrives', async () => {
        ...
        const { result, rerender } = renderHook(({ session }) => useAgentChat(SessionId('session-1'), session, SupportedModelId('claude-sonnet-4-5'), 'medium'), {
