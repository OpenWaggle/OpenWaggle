...
import { apiMock, createSession, createSessionWithId, emitAgentEvent, emitRunCompleted, installUseAgentChatTestLifecycle, SEND_PAYLOAD, useAgentChat, } from './useAgentChat.test-utils';
describe('useAgentChat streaming', () => {
    installUseAgentChatTestLifecycle();
    it('applies stream text immediately as chunks arrive', async () => {
        const { result } = renderHook(() => useAgentChat(SessionId('session-1'), createSession(), SupportedModelId('claude-sonnet-4-5'), 'medium'));
    ...
    it('tracks pending Pi interactions and submits typed responses', async () => {
        ...
        expect(result.current.agentInteractionEvents[0]).toEqual(expect.objectContaining({ type: 'agent_interaction_request' }));
        await act(async () => {
        ...
        expect(result.current.agentInteractionEvents[1]).toEqual(expect.objectContaining({ type: 'agent_interaction_resolved' }));
    });
    it('clears pending interactions resolved while another session is selected', async () => {
        ...
        const { result, rerender } = renderHook(({ sessionId }) => useAgentChat(sessionId, createSessionWithId(sessionId), SupportedModelId('claude-sonnet-4-5'), 'medium'), { initialProps: { sessionId: sessionA } });
        await act(async () => {
            emitAgentEvent({
                sessionId: sessionA,
                event: {
