...
function requireAgentEventHandler() {
    ...
    if (!handler)
        throw new Error('Expected agent event handler');
    return handler;
}
function requireRunCompletedHandler() {
    const handler = apiMock.getRunCompletedHandler();
    if (!handler)
        throw new Error('Expected run-completed handler');
    return handler;
}
...
function phaseHandle(current = null) {
    return {
        current,
        completed: [],
        totalElapsedMs: 0,
...
function sendWorkflowParams(overrides = {}) {
    const params = {
...
describe('chat orchestration hooks', () => {
    ...
    it('tracks background run lifecycle events and clears snapshots after completion refresh', async () => {
        ...
        await waitFor(() => expect(useBackgroundRunStore.getState().hasActiveRun(SESSION_ID)).toBe(true));
        ...
        await waitFor(() => expect(useBackgroundRunStore.getState().getRunRenderSnapshot(SESSION_ID)).toBeNull());
        unmount();
    ...
    it('does not swallow first-message Waggle sends before a session exists', async () => {
        ...
        expect(params.handleSendWaggle).toHaveBeenCalledWith(payload('Run the same prompt again'), config);
    ...
    it('builds composer section state and inserts skill commands without a mounted Lexical editor', () => {
        ...
        const { result } = renderHook(() => useComposerSection({
            isLoading: false,
            isSteering: false,
            status: 'ready',
    ...
    it('keeps session copy commands safe when there is no active session or fork target', async () => {
        ...
        const { result } = renderHook(() => useSessionCopyWorkflow({
            activeSessionId: null,
