...
import { apiMock, createDeferred, createSession, createSessionWithMessages, emitAgentEvent, hasActiveRunMock, installUseAgentChatTestLifecycle, useAgentChat, } from './useAgentChat.test-utils';
describe('useAgentChat background reconnect', () => {
    ...
    it('reconnects to an active background run snapshot', async () => {
        ...
        const { result } = renderHook(() => useAgentChat(SessionId('session-1'), session, SupportedModelId('claude-sonnet-4-5'), 'medium'));
        await waitFor(() => {
    ...
    it('merges background reconnect snapshots when stream events arrive before reconnect completes', async () => {
        ...
        apiMock.getSessionDetail.mockResolvedValue(createSessionWithMessages(1, [
            {
                id: MessageId('user-1'),
                role: 'user',
                createdAt: 1,
        ...
        const { result } = renderHook(() => useAgentChat(SessionId('session-1'), createSessionWithMessages(1, [
            {
                id: MessageId('user-1'),
                role: 'user',
                createdAt: 1,
        ...
        ]), SupportedModelId('claude-sonnet-4-5'), 'medium'));
        await waitFor(() => {
            expect(result.current.backgroundStreaming).toBe(true);
        });
        await act(async () => {
