...
function makePayload(text) {
    return { text, thinkingLevel: THINKING, attachments: [] };
}
...
describe('useAutoSendQueue', () => {
    ...
    it('fires when status transitions from streaming to ready', () => {
        ...
        const { rerender } = renderHook(({ status }) => useAutoSendQueue({ sessionId: CONV_A, status, sendMessage: send }), { initialProps: { status: 'streaming' } });
    ...
    it('does NOT fire when sessionId is null', () => {
        const send = vi.fn().mockResolvedValue(undefined);
        useMessageQueueStore.getState().enqueue(CONV_A, makePayload('test'));
        const { rerender } = renderHook(({ status }) => useAutoSendQueue({ sessionId: null, status, sendMessage: send }), { initialProps: { status: 'streaming' } });
        rerender({ status: 'ready' });
        expect(send).not.toHaveBeenCalled();
    });
    it('does NOT fire when paused even on valid transition', () => {
        const send = vi.fn().mockResolvedValue(undefined);
        useMessageQueueStore.getState().enqueue(CONV_A, makePayload('test'));
        const { rerender } = renderHook(({ status, paused }) => useAutoSendQueue({ sessionId: CONV_A, status, sendMessage: send, paused }), { initialProps: { status: 'streaming', paused: true } });
    ...
    it('fires when unpaused after a paused transition', () => {
        ...
        const { rerender } = renderHook(({ status, paused }) => useAutoSendQueue({ sessionId: CONV_A, status, sendMessage: send, paused }), { initialProps: { status: 'streaming', paused: false } });
    ...
    it('does not auto-send newly queued messages while the agent is still streaming', () => {
        const send = vi.fn().mockResolvedValue(undefined);
        renderHook(() => useAutoSendQueue({ sessionId: CONV_A, status: 'streaming', sendMessage: send }));
    ...
    it('re-enqueues and reports send failures', async () => {
        ...
        const { rerender } = renderHook(({ status }) => useAutoSendQueue({
            sessionId: CONV_A,
            status,
            sendMessage: send,
            onSendFailure,
    ...
    it('reports send failure through the callback captured for the original session', async () => {
        ...
        const { rerender } = renderHook(({ sessionId, status, onSendFailure, }) => useAutoSendQueue({
            sessionId,
            status,
            sendMessage: send,
            onSendFailure,
