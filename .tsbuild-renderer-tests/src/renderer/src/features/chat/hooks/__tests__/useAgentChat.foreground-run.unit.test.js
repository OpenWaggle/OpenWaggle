...
import { apiMock, createDeferred, createSession, createSessionWithId, emitAgentEvent, emitRunCompleted, installUseAgentChatTestLifecycle, SEND_PAYLOAD, useAgentChat, } from './useAgentChat.test-utils';
describe('useAgentChat foreground run', () => {
    ...
    it('streams optimistic user and assistant text through OpenWaggle runtime events', async () => {
        ...
        expect(apiMock.sendMessage).toHaveBeenCalledWith(SessionId('session-1'), SEND_PAYLOAD, SupportedModelId('claude-sonnet-4-5'));
    ...
    it('surfaces send failures and clears the foreground run loading state', async () => {
        ...
        const { result } = renderHook(() => useAgentChat(SessionId('session-1'), createSession(), SupportedModelId('claude-sonnet-4-5'), 'medium'));
        await act(async () => {
            await expect(result.current.sendMessage(SEND_PAYLOAD)).rejects.toThrow('Invalid arguments for "agent:send-message"');
        });
