...
import { apiMock, createDeferred, createSessionWithMessages, emitAgentEvent, hasActiveRunMock, installUseAgentChatTestLifecycle, runRenderSnapshots, useAgentChat, } from './useAgentChat.test-utils';
describe('useAgentChat reconnect', () => {
    ...
    it('keeps an optimistic user message when reconnecting after a route remount', async () => {
        ...
        const { result } = renderHook(() => useAgentChat(SessionId('session-1'), createSessionWithMessages(1, []), SupportedModelId('claude-sonnet-4-5'), 'medium'));
        await waitFor(() => {
    ...
    it('uses the latest persisted session snapshot when reconnecting to a background run', async () => {
        ...
        apiMock.getSessionDetail.mockResolvedValue(createSessionWithMessages(2, [
            {
                id: MessageId('user-1'),
                role: 'user',
                createdAt: 1,
