...
import { createDefaultParams, createTestLayout, flushAnimationFrame, installChatScrollTestEnvironment, renderScrollHook, triggerObservedResize, } from './useChatScrollBehaviour.test-utils';
describe('useChatScrollBehaviour user intent behavior', () => {
    ...
    it('cancels a queued stream follow immediately when the user wheels upward', () => {
        ...
        const { result, rerender } = renderScrollHook(createDefaultParams({ isLoading: true, rowsLength: 2, streamVersion: 10 }), layout);
    ...
    it('opts out for pointer scrollbar drags and touch upward gestures', () => {
        ...
        const touchHook = renderScrollHook(createDefaultParams({ activeSessionId: 'session-touch' }), touchLayout);
