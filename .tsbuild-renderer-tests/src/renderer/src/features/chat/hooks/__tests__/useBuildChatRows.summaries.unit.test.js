...
import { buildChatRows, createUserMessage, SessionBranchId, SessionId, SupportedModelId, } from './useBuildChatRows.test-utils';
...
describe('buildChatRows reasoning visibility', () => {
    it('keeps assistant rows that contain inline reasoning content', () => {
        ...
        const assistantRows = rows.filter((row) => row.type === 'message' && row.message.role === 'assistant');
    ...
    it('sets isRunActive on the last assistant row when isLoading is true', () => {
        const messages = [
            createUserMessage('user-1', 'hello'),
            {
                id: 'assistant-1',
                role: 'assistant',
                parts: [{ type: 'text', content: 'first reply' }],
            },
            {
                id: 'assistant-2',
                role: 'assistant',
                parts: [{ type: 'text', content: 'second reply' }],
            },
        ];
        const rows = buildChatRows({
            messages,
