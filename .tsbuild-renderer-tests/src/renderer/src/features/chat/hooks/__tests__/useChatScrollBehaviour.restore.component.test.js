...
import { createDefaultParams, createTestLayout, expectScrollCacheEntry, installChatScrollTestEnvironment, renderScrollHook, SCROLL_PERSIST_DEBOUNCE_MS, triggerObservedResize, } from './useChatScrollBehaviour.test-utils';
describe('useChatScrollBehaviour session restoration behavior', () => {
    ...
    it('restores the scroll position captured from the previous session scroll event', () => {
        ...
        const { result, rerender } = renderScrollHook(createDefaultParams({ activeSessionId: 'session-a', lastUserMessageId: 'user-a' }), layout);
