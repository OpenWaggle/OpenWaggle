...
describe('useIncrementalMarkdown', () => {
    ...
    it('incrementally extends prefix HAST when new paragraphs arrive', () => {
        ...
        const { result, rerender } = renderHook(({ t }) => useIncrementalMarkdown(t, true, SHIKI_OPTIONS), { initialProps: { t: text } });
        const firstHast = result.current.prefixHast;
    ...
    it('invalidates prefix cache when highlighter changes', () => {
        ...
        const { result, rerender } = renderHook(({ opts }) => useIncrementalMarkdown(text, true, opts), { initialProps: { opts: options1 } });
        const firstHast = result.current.prefixHast;
        // Simulate highlighter becoming available
        const fakeHighlighter = fromPartial({});
