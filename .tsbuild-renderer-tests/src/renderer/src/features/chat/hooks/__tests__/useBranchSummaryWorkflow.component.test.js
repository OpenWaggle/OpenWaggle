...
describe('useBranchSummaryWorkflow', () => {
    ...
    it('summarizes a draft branch with trimmed custom instructions and restores the active composer draft', async () => {
        ...
        expect(branchSummaryMocks.navigateSessionTree).toHaveBeenCalledWith(SESSION_ID, MODEL, SOURCE_NODE_ID, { summarize: true, customInstructions: 'keep architecture rationale' });
    ...
    it('cancels branch summarization by restoring the original composer context', () => {
        openPrompt();
        const params = workflowParams();
        useComposerStore.getState().switchScopedDraftContext(buildComposerDraftContextKey({
            projectPath: '/repo',
            sessionId: SESSION_ID,
