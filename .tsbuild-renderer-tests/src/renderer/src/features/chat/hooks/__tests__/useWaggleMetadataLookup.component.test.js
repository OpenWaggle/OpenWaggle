...
import { makeAssistantMessage, makeConfig, makeProjectedSession, makeSessionDetail, makeUserMessage, } from './useWaggleMetadataLookup.test-utils';
describe('useWaggleMetadataLookup', () => {
    ...
    it('returns equivalent metadata when inputs and waggle state are unchanged', () => {
        ...
        const { result, rerender } = renderHook(({ currentSession, currentMessages, }) => useWaggleMetadataLookup(currentSession, currentMessages), { initialProps: { currentSession: session, currentMessages: messages } });
