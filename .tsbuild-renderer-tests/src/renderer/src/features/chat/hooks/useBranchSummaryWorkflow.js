...
function draftBranchComposerContextKey(params, sessionId, sourceNodeId) {
...
function routeToSessionSelection(params, sessionId, selection) {
...
function isCurrentBranchSummaryPrompt(prompt) {
    ...
    return (currentPrompt?.sessionId === prompt.sessionId &&
...
function restoreBranchSummaryChoice(prompt, previousMode) {
...
function switchComposerToDraftBranch(params, input) {
...
async function refreshAfterBranchSummary(params, prompt) {
    await Promise.all([
        params.loadSessions(),
...
function applySummarizedBranchDraft(params, prompt) {
    ...
    const appliedDraft = useComposerStore
        .getState()
        .switchScopedDraftContext(contextKey, { input: prompt.draftComposerText, attachments: [] }, { input: prompt.draftComposerText, attachments: useComposerStore.getState().attachments });
    useComposerStore
        .getState()
...
async function navigateWithBranchSummary(params, prompt, customInstructions) {
...
async function finishBranchSummary(params, prompt) {
...
function cancelBranchSummary(prompt, previousMode) {
    restoreBranchSummaryChoice(prompt, previousMode);
}
async function materializeBranchSummaryAction(params, customInstructions) {
...
export function useBranchSummaryWorkflow(params) {
    return {
        async materializeBranchSummary(customInstructions) {
            await materializeBranchSummaryAction(params, customInstructions);
        },
        async materializeDraftBranchForSend(draftBranch) {
            if (!params.activeSessionId)
                return true;
            if (draftBranch?.sessionId !== params.activeSessionId)
                return true;
            const navigation = await api.navigateSessionTree(params.activeSessionId, params.model, draftBranch.sourceNodeId);
            if (navigation.cancelled) {
        ...
        cancelBranchSummary() {
            ...
            const appliedDraft = useComposerStore
                .getState()
                .switchScopedDraftContext(restoreContextKey, { input: prompt.previousComposerText, attachments: [] }, { input: '', attachments: [] });
            useComposerStore
                .getState()
                .clearScopedDraft(draftBranchComposerContextKey(params, prompt.sessionId, prompt.sourceNodeId));
            useBranchSummaryStore.getState().clearPrompt();
        ...
        switchComposerToDraftBranch(input) {
