...
import { getMaxScrollTop, isScrollContainerNearBottom, scrollElementToBottom, } from '@/features/chat/lib/scroll-to-bottom';
import { SCROLL_PERSIST_DEBOUNCE_MS, SCROLL_UP_HYSTERESIS_PX, SCROLLBAR_HIDE_DELAY_MS, SESSION_RESTORE_RETRY_MS, saveScrollCache, } from './chat-scroll-cache';
import { useChatScrollEffects } from './useChatScrollEffects';
import { useChatScrollRefs } from './useChatScrollRefs';
function scrolledUpBeyondHysteresis(currentScrollTop, lastKnownScrollTop) {
    return currentScrollTop < lastKnownScrollTop - SCROLL_UP_HYSTERESIS_PX;
}
function resolveAutoScrollSnapshot(snapshot) {
    ...
    const userDetachedFromBottom = snapshot.shouldAutoScroll &&
...
export function useChatScrollBehaviour(params) {
    const { activeSessionId, rowsLength, streamVersion, isLoading, userDidSend, onUserDidSendConsumed, } = params;
    const { lastUserMessageId } = params;
    const { scrollerRef, contentRef, shouldAutoScrollRef, lastKnownScrollTopRef, isPointerScrollActiveRef, lastTouchClientYRef, pendingUserScrollUpIntentRef, pendingAutoScrollFrameRef, pendingRestoreTimerRef, pendingRestoreScrollTopRef, activeSessionIdRef, scrollCacheRef, persistTimerRef, scrollbarTimerRef, actionsRef, effectRefs, } = useChatScrollRefs(activeSessionId, lastUserMessageId);
    ...
    function rememberScrollPosition(sessionId, scrollTop) {
        if (!sessionId)
            return;
    ...
    function applyPendingRestore() {
        ...
        if (!scrollContainer || target === null)
            return false;
    ...
    function handleScroll() {
        ...
        if (nextAutoScroll.shouldCancelPendingStickToBottom)
            cancelPendingStickToBottom();
