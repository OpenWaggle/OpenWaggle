...
vi.mock('@/shared/lib/ipc', () => ({
    api: {
        listSessionDetails: (...args) => mockApi.listSessionDetails(...args),
        listSessions: (...args) => mockApi.listSessions(...args),
        getSessionTree: (...args) => mockApi.getSessionTree(...args),
        getSessionDetail: (...args) => mockApi.getSessionDetail(...args),
        createSession: (...args) => mockApi.createSession(...args),
        deleteSession: (...args) => mockApi.deleteSession(...args),
    },
}));
...
function makeSessionDetail(id, title = 'Session') {
    return {
        id,
        title,
        projectPath: '/repo',
...
describe('useChatStore integration', () => {
    ...
    it('throws and preserves state on createSession failure', async () => {
        ...
        await expect(useChatStore.getState().createSession('/tmp/repo')).rejects.toThrow('quota exceeded');
        expect(useChatStore.getState().activeSessionId).toBeNull();
    });
});
