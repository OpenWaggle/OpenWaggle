...
describe('Waggle indicators', () => {
    it('renders turn labels with agent model metadata', () => {
        render(_jsx(TurnDivider, { turnNumber: 1, agentLabel: "Reviewer", agentColor: "blue", agentModel: SupportedModelId('openai/gpt-5.5') }));
        expect(screen.getByText('Turn 2: Reviewer')).toBeInTheDocument();
        expect(screen.getByText(/GPT 5.5/)).toBeInTheDocument();
    });
    it('renders the bee icon as decorative svg', () => {
        const { container } = render(_jsx(WaggleBeeIcon, { className: "size-4" }));
