...
describe('composer-store', () => {
    ...
    describe('scoped drafts', () => {
        it('saves the current context and loads the fallback draft when switching contexts', () => {
            ...
            expect(useComposerStore.getState().getScopedDraft('project:/tmp:session:s1:branch:main')).toEqual({ input: 'main draft', attachments: [attachment] });
        });
        ...
        it('removes the active scoped draft on reset after send', () => {
            ...
            useComposerStore.getState().reset();
            expect(useComposerStore.getState().getScopedDraft('project:/tmp:session:s1:branch:main')).toBeNull();
        });
        ...
        it('clears visible draft state when clearing drafts for the active branch', () => {
            ...
            expect(useComposerStore.getState().input).toBe('');
            expect(useComposerStore.getState().attachments).toEqual([]);
            expect(useComposerStore.getState().getScopedDraft('project:/tmp:session:s1:branch:b1')).toBeNull();
        });
    });
    describe('reset', () => {
