...
export function downsampleAudio(samples, sourceRate, targetRate) {
...
export function trimSilence(samples, sampleRate, threshold = DEFAULT_SILENCE_THRESHOLD, paddingMs = DEFAULT_SILENCE_PADDING_MS) {
    if (samples.length === 0)
        return samples;
    let startIndex = 0;
    ...
    return samples.slice(Math.max(0, startIndex - paddingSamples), Math.min(samples.length, endIndex + paddingSamples + 1));
}
export function toPcm16(samples) {
    ...
    for (let index = 0; index < samples.length; index += 1) {
        ...
        view.setInt16(index * DOUBLE_FACTOR, sample < 0
            ? Math.round(sample * PCM16_NEGATIVE_SCALE)
            : Math.round(sample * PCM16_POSITIVE_SCALE), true);
    }
    return bytes;
}
