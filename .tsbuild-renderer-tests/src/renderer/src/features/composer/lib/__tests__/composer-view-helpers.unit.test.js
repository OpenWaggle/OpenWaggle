...
import { buildContextMeterValue, buildContextUsageRequestKey, findContextWindow, } from '../context-meter-view';
import { findMentionMatch } from '../mention-match';
import { getThinkingButtonLabel, getThinkingButtonTitle, hasOnlyOffThinkingLevel, } from '../thinking-level-view';
...
describe('composer view helpers', () => {
    ...
    it('resolves context usage request keys and model context windows', () => {
        expect(buildContextUsageRequestKey('session-1', MODEL_REF, 'v1')).toBe('session-1:anthropic/claude-sonnet-4-5:v1');
