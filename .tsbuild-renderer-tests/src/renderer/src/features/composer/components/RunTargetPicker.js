 ...
 */
export function RunTargetPicker({ strip, onToast }) {
     ...
     */
    const worktreeBranch = useGitStore((state) => strip?.worktreePath == null
        ? null
    ...
    return (_jsxs(Popover, { open: controller.branchMenuOpen, onOpenChange: (open) => controller.openMenu(open ? 'branch' : null), placement: "top-end", className: "w-[320px] p-2", trigger: _jsx(RunTargetTrigger, { selectedRef: selectedRef, isOpen: controller.branchMenuOpen, isMissing: isMissing, onToggle: (open) => controller.openMenu(open ? 'branch' : null) }), children: [_jsx(BranchPickerSearch, { query: controller.branchQuery, isBranchActionRunning: controller.isBranchActionRunning, onQueryChange: controller.setBranchQuery }), _jsx(BranchPickerList, { filteredBranches: controller.filteredBranches, localBranches: controller.localBranches, remoteBranches: controller.remoteBranches, selectedRef: selectedRef, onSelectRef: selectRef }), _jsx(RunTargetOptions, { strip: strip, selectedRef: selectedRef, onOpenActionDialog: controller.openActionDialog, onToast: onToast })] }));
}
