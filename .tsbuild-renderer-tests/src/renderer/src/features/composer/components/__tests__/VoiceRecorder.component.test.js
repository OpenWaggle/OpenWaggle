...
vi.mock('react-voice-visualizer', () => ({
    VoiceVisualizer: () => _jsx("div", { "data-testid": "voice-visualizer" }),
}));
...
function createVoiceController(overrides = {}) {
    return {
        canStart: false,
        clearError: vi.fn(),
        elapsedSeconds: 9,
...
describe('VoiceRecorder', () => {
    ...
    it('renders the compact recording row with timer and actions', () => {
        render(_jsx(VoiceRecorder, { fileInputRef: createFileInputRef(), voice: createVoiceController({ elapsedSeconds: 9 }) }));
    ...
    it('calls stop and send actions from the inline controls', () => {
        ...
        render(_jsx(VoiceRecorder, { fileInputRef: createFileInputRef(), voice: voice }));
    ...
    it('shows the transcribing spinner state and disables send', () => {
        render(_jsx(VoiceRecorder, { fileInputRef: createFileInputRef(), voice: createVoiceController({ mode: 'transcribing' }) }));
