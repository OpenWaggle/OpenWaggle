...
describe('QueuedMessages', () => {
    ...
    it('renders nothing when the queue is empty', () => {
        const { container } = render(_jsx(QueuedMessages, { sessionId: CONV_A, onSteer: noOpSteer, isStreaming: false }));
        expect(container.firstChild).toBeNull();
    });
    it('renders nothing when sessionId is null', () => {
        useMessageQueueStore.getState().enqueue(CONV_A, makePayload('test'));
        const { container } = render(_jsx(QueuedMessages, { sessionId: null, onSteer: noOpSteer, isStreaming: false }));
        expect(container.firstChild).toBeNull();
    });
    it('renders header with count badge', () => {
        ...
        render(_jsx(QueuedMessages, { sessionId: CONV_A, onSteer: noOpSteer, isStreaming: false }));
    ...
    it('shows Steer button only when isStreaming is true', () => {
        useMessageQueueStore.getState().enqueue(CONV_A, makePayload('test'));
        const { rerender } = render(_jsx(QueuedMessages, { sessionId: CONV_A, onSteer: noOpSteer, isStreaming: false }));
        expect(screen.queryByText('Steer')).not.toBeInTheDocument();
        rerender(_jsx(QueuedMessages, { sessionId: CONV_A, onSteer: noOpSteer, isStreaming: true }));
    ...
    it('uses compaction copy and hides Steer while compaction is running', () => {
        ...
        render(_jsx(QueuedMessages, { sessionId: CONV_A, onSteer: noOpSteer, isStreaming: true, isCompacting: true }));
    ...
    it('Steer button calls onSteer with correct messageId', () => {
        ...
        render(_jsx(QueuedMessages, { sessionId: CONV_A, onSteer: noOpSteer, isStreaming: true }));
