...
vi.mock('@/shared/lib/ipc', () => ({
    api: {
        prepareAttachments: (...args) => prepareAttachmentsMock(...args),
    },
}));
...
function createDragEvent(files = []) {
    return fromAny({
...
describe('useFileAttachment', () => {
    ...
    it('sets error when no project path on drop', async () => {
        ...
        expect(params.setAttachmentError).toHaveBeenCalledWith('Select a project before attaching files.');
    });
