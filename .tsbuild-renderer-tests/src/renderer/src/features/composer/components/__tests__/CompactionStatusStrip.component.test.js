...
describe('CompactionStatusStrip', () => {
    it.each(COMPACTION_CASES)('renders Pi-style compaction copy for %s compaction', (reason, label) => {
        render(_jsx(CompactionStatusStrip, { state: { type: 'compacting', reason }, onCancel: vi.fn() }));
    ...
    it('renders retry feedback and cancels through the same stop action', () => {
        const cancel = vi.fn();
        render(_jsx(CompactionStatusStrip, { state: {
                type: 'retrying',
                attempt: 1,
                maxAttempts: 2,
                delayMs: 2500,
