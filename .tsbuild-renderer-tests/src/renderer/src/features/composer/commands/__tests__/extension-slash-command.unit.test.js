...
import { extensionSlashCommandPayload, extensionSlashCommandText, invokableExtensionSlashCommandEntries, parseExtensionSlashCommand, } from '../extension-slash-command';
function slashEntry(overrides = {}) {
    return {
...
describe('extension slash command parsing', () => {
    it('lists only eligible slash command contributions with broker invocation targets', () => {
        ...
        expect(invokableExtensionSlashCommandEntries(registry([
            slashEntry(),
            ...
            disabledEntry,
        ])).map((entry) => entry.contributionId)).toEqual(['sample.run']);
    });
    it('parses a submitted extension slash command and preserves trailing args', () => {
        const command = parseExtensionSlashCommand('  /sample.run use current diff  ', registry([slashEntry()]));
