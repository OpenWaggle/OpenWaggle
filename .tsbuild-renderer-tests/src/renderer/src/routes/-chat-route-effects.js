...
function sessionIdFromRoute(sessionId) {
    return SessionId(sessionId);
}
export function useChatRouteEffects({ branchId, diffOpen, nodeId, sessionId, }) {
    ...
    const routeSessionDetail = useChatStore((state) => routeSessionId === null ? null : (state.sessionById.get(routeSessionId) ?? null));
    const routeSessionMissing = useChatStore((state) => routeSessionId === null ? false : state.missingSessionIds.has(routeSessionId));
