import type { ConversationId } from '@shared/types/brand'
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { useThreadStatusStore } from '../thread-status-store'

const ID_A = 'conv-a' as ConversationId
const ID_B = 'conv-b' as ConversationId

describe('thread-status-store', () => {
  beforeEach(() => {
    useThreadStatusStore.setState({
      statuses: new Map(),
      completedAt: new Map(),
      lastVisitedAt: new Map(),
    })
  })

  afterEach(() => {
    vi.restoreAllMocks()
  })

  describe('setStatus with completedAt tracking', () => {
    it('records completedAt when setting a terminal status', () => {
      const now = 1000
      vi.spyOn(Date, 'now').mockReturnValue(now)

      useThreadStatusStore.getState().setStatus(ID_A, 'completed')

      expect(useThreadStatusStore.getState().statuses.get(ID_A)).toBe('completed')
      expect(useThreadStatusStore.getState().completedAt.get(ID_A)).toBe(now)
    })

    it('records completedAt for error status', () => {
      const now = 2000
      vi.spyOn(Date, 'now').mockReturnValue(now)

      useThreadStatusStore.getState().setStatus(ID_A, 'error')

      expect(useThreadStatusStore.getState().statuses.get(ID_A)).toBe('error')
      expect(useThreadStatusStore.getState().completedAt.get(ID_A)).toBe(now)
    })

    it('clears completedAt when setting a live status', () => {
      vi.spyOn(Date, 'now').mockReturnValue(1000)
      useThreadStatusStore.getState().setStatus(ID_A, 'completed')
      expect(useThreadStatusStore.getState().completedAt.has(ID_A)).toBe(true)

      useThreadStatusStore.getState().setStatus(ID_A, 'working')
      expect(useThreadStatusStore.getState().completedAt.has(ID_A)).toBe(false)
    })

    it('clears completedAt when setting idle', () => {
      vi.spyOn(Date, 'now').mockReturnValue(1000)
      useThreadStatusStore.getState().setStatus(ID_A, 'error')
      expect(useThreadStatusStore.getState().completedAt.has(ID_A)).toBe(true)

      useThreadStatusStore.getState().setStatus(ID_A, 'idle')
      expect(useThreadStatusStore.getState().completedAt.has(ID_A)).toBe(false)
      expect(useThreadStatusStore.getState().statuses.has(ID_A)).toBe(false)
    })
  })

  describe('markVisited', () => {
    it('sets lastVisitedAt to current time', () => {
      const now = 5000
      vi.spyOn(Date, 'now').mockReturnValue(now)

      useThreadStatusStore.getState().markVisited(ID_A)

      expect(useThreadStatusStore.getState().lastVisitedAt.get(ID_A)).toBe(now)
    })
  })

  describe('markUnread', () => {
    it('sets lastVisitedAt to completedAt - 1 when completedAt exists', () => {
      vi.spyOn(Date, 'now').mockReturnValue(1000)
      useThreadStatusStore.getState().setStatus(ID_A, 'completed')

      vi.spyOn(Date, 'now').mockReturnValue(2000)
      useThreadStatusStore.getState().markVisited(ID_A)

      // Now the icon is "seen" — markUnread should re-show it
      useThreadStatusStore.getState().markUnread(ID_A)

      const completedAt = useThreadStatusStore.getState().completedAt.get(ID_A)
      const lastVisited = useThreadStatusStore.getState().lastVisitedAt.get(ID_A)

      expect(completedAt).toBe(1000)
      expect(lastVisited).toBe(999) // completedAt - 1
    })

    it('uses Date.now() - 1 when completedAt does not exist', () => {
      const now = 3000
      vi.spyOn(Date, 'now').mockReturnValue(now)

      useThreadStatusStore.getState().markUnread(ID_B)

      expect(useThreadStatusStore.getState().lastVisitedAt.get(ID_B)).toBe(now - 1)
    })
  })

  describe('terminal seen/unseen logic', () => {
    it('terminal status is unseen when no visit recorded', () => {
      vi.spyOn(Date, 'now').mockReturnValue(1000)
      useThreadStatusStore.getState().setStatus(ID_A, 'completed')

      const state = useThreadStatusStore.getState()
      const completedAt = state.completedAt.get(ID_A)
      const lastVisited = state.lastVisitedAt.get(ID_A)

      // No visit → unseen
      expect(completedAt).toBeDefined()
      expect(lastVisited).toBeUndefined()
    })

    it('terminal status becomes seen after visit', () => {
      vi.spyOn(Date, 'now').mockReturnValue(1000)
      useThreadStatusStore.getState().setStatus(ID_A, 'completed')

      vi.spyOn(Date, 'now').mockReturnValue(2000)
      useThreadStatusStore.getState().markVisited(ID_A)

      const state = useThreadStatusStore.getState()
      const completedAt = state.completedAt.get(ID_A)
      const lastVisited = state.lastVisitedAt.get(ID_A)

      expect(completedAt).toBe(1000)
      expect(lastVisited).toBe(2000)
      // completedAt <= lastVisited → seen
      expect(completedAt).toBeDefined()
      expect(lastVisited).toBeDefined()
      expect(completedAt).toBeLessThanOrEqual(lastVisited ?? 0)
    })

    it('markUnread makes a seen terminal status unseen again', () => {
      vi.spyOn(Date, 'now').mockReturnValue(1000)
      useThreadStatusStore.getState().setStatus(ID_A, 'error')

      vi.spyOn(Date, 'now').mockReturnValue(2000)
      useThreadStatusStore.getState().markVisited(ID_A)

      useThreadStatusStore.getState().markUnread(ID_A)

      const state = useThreadStatusStore.getState()
      const completedAt = state.completedAt.get(ID_A)
      const lastVisited = state.lastVisitedAt.get(ID_A)

      // completedAt > lastVisited → unseen
      expect(completedAt).toBeDefined()
      expect(lastVisited).toBeDefined()
      expect(completedAt).toBeGreaterThan(lastVisited ?? 0)
    })
  })

  describe('clearStatus', () => {
    it('removes both status and completedAt', () => {
      vi.spyOn(Date, 'now').mockReturnValue(1000)
      useThreadStatusStore.getState().setStatus(ID_A, 'completed')

      useThreadStatusStore.getState().clearStatus(ID_A)

      expect(useThreadStatusStore.getState().statuses.has(ID_A)).toBe(false)
      expect(useThreadStatusStore.getState().completedAt.has(ID_A)).toBe(false)
    })
  })
})
