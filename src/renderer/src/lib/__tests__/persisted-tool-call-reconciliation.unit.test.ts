...
function makeDuplicatePersistedConversation(): Conversation {
  return {
    ...makePersistedConversation(),
    messages: [
      ...makePersistedConversation().messages,
      {
        id: MessageId('msg-2'),
        role: 'assistant',
        createdAt: 3,
...
describe('restorePersistedToolCallPart', () => {
  ...
  it('restores richer arguments when a later duplicate persisted tool call lost its args', () => {
    const lookup = buildPersistedToolCallLookup(makeDuplicatePersistedConversation())
    const uiPart: UIToolCallPart = {
      type: 'tool-call',
      id: 'tool-persisted',
      name: 'writeFile',
