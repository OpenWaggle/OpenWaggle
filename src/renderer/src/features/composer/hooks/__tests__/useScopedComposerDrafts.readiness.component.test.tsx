...
describe('scoped composer draft hydration', () => {
  ...
  it('makes the owned pending draft editable and restores an untouched saved branch draft', async () => {
    ...
    expect(useComposerStore.getState().activeDraftContextKey).toBe(`session:${SESSION_ID}:pending`)
  ...
  it('binds each new session selection to its pending owner and preserves both restored drafts', async () => {
