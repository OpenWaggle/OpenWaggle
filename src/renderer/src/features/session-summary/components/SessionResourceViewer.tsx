...
function ViewerCanvas({
  ...
  return (
    <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-auto bg-bg-tertiary p-8">
      <Button
        variant="secondary"
        size="icon-sm"
        aria-label="Previous image"
        disabled={index <= 0}
        className="absolute left-5 top-1/2 z-10"
      ...
      <Button
        ...
        className="absolute right-5 top-1/2 z-10"
        onClick={() => onNavigate(index + 1)}
      >
...
function useViewerSource(sessionId: string | null, resource: SessionResource | null) {
  const content = useQuery({
    ...
    enabled: resource?.locator?.startsWith('session-resource://') === true,
  })
