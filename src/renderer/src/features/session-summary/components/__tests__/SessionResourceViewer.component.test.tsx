...
describe('SessionResourceViewer', () => {
  ...
  it('shows the selected image classification and latest branch provenance', async () => {
    listSessionResources.mockResolvedValue([
      {
        ...image('image-1', 'first.png'),
        isOutput: true,
        occurrences: [
          ...
          },
          {
            ...
            branchId: 'session-1:feature/gallery',
            actor: 'agent',
            activity: 'updated',
            label: null,
            createdAt: 1100,
    ...
    await screen.findByRole('dialog', { name: 'Image viewer: first.png' })
    expect(screen.getByLabelText('Image provenance')).toHaveTextContent(
      '1 of 1 · Source and output · Updated by the agent · Branch feature/gallery',
    )
  })
