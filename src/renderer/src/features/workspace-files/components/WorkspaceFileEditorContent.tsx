...
function LineEndingNormalization({ editing }: { readonly editing: WorkspaceFileEditing }) {
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="border-b border-warning/30 bg-warning/10 p-3 text-xs text-text-secondary">
        ...
        <div className="mt-2 flex gap-2">
          <Button
            size="xs"
            variant="accent"
            onClick={() => void editing.normalizeLineEndings('lf')}
          >
            Normalize to LF
          </Button>
          <Button
            size="xs"
            variant="ghost"
            onClick={() => void editing.normalizeLineEndings('crlf')}
          >
            Normalize to CRLF
          </Button>
        </div>
...
export function WorkspaceFileEditorContent({
  ...
  if (focusedEdit && editing.normalizationRequired)
    return <LineEndingNormalization editing={editing} />
