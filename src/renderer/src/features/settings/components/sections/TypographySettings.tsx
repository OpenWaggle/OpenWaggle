...
export function TypographySettings() {
  ...
  function update(patch: Partial<AppearanceTypographyPreferences>) {
    void setTypography(patch).catch((error: unknown) => {
      showToast(error instanceof Error ? error.message : 'Could not save typography.', 'error')
    })
  }
    ...
    <section className="space-y-3" aria-labelledby="typography-heading">
      <div className="flex items-start justify-between gap-4">
        ...
        <Button
          ...
          onClick={() => update(DEFAULT_APPEARANCE_TYPOGRAPHY)}
        >
          Reset
        </Button>
      </div>
