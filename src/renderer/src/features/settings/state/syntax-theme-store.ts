...
async function validatedCatalogLanguages(catalog: SyntaxResourceCatalog) {
  const languages = activatableSyntaxLanguageResources(catalog.languages)
  await validateImportedSyntaxLanguages(languages)
  return languages
}
...
export const useSyntaxThemeCatalogStore = create<SyntaxThemeCatalogState>((set, get) => ({
  ...
  load: async (projectPath = null) => {
    ...
    try {
      ...
      const languages = await validatedCatalogLanguages(catalog)
      if (loadId !== latestCatalogLoad) return
