/**
 * Every method the preload bridge exposes.
 *
 * Kept beside the test rather than inside it, like PRELOAD_MCP_METHODS, so the contract list can
 * grow without pushing the test file past its line budget.
 */
import type { OpenWaggleApi } from '@shared/types/openwaggle-api'
import { PRELOAD_MCP_METHODS } from './preload-mcp-methods'

export const PRELOAD_API_METHODS: readonly (keyof OpenWaggleApi)[] = [
  'getCliShimStatus',
  'installCliShim',
  'removeCliShim',
  'selectAgentDefinitionSource',
  'manageAgentDefinitions',
  'listAgentDefinitionDisplay',
  'getAgentDefinitionPreview',
  'setAgentDefinitionEnabled',
  'manageAccessProfiles',
  'mutateSessionControl',
  'querySessionControl',
  // Agent
  'sendMessage',
  'cancelAgent',
  'respondAgentInteraction',
  'onAgentEvent',
  'getAgentPhase',
  'getBackgroundRun',
  'listActiveRuns',
  'getContextUsage',
  'compactSession',
  'onRunCompleted',
  'onSessionResourcesInvalidated',
  'activateSessionResourceOwner',
  'onAgentPhase',
  'onSessionHostEvent',
  'onSessionHostResyncRequired',
  'onWorktreeLaunch',
  // Settings
  'getSettings',
  'updateSettings',
  'setEnabledModels',
  'getPiTreeFilterMode',
  'setPiTreeFilterMode',
  'getPiBranchSummarySkipPrompt',
  'testApiKey',
  ...PRELOAD_MCP_METHODS,
  'listExtensionPackages',
  'listExtensionContributions',
  'proposeExtensionPackageWrite',
  'applyExtensionPackageWrite',
  'proposeExtensionPackageRemove',
  'applyExtensionPackageRemove',
  'invokeExtension',
  'registerExtensionFrame',
  'unregisterExtensionFrame',
  'registerInlineVisualizationFrame',
  'unregisterInlineVisualizationFrame',
  'saveInlineVisualizationDownload',
  'setExtensionTrusted',
  'setExtensionEnabled',
  'setExtensionProjectDisabled',
  'acceptExtensionUpdate',
  'approveExtensionBuild',
  'reloadExtension',
  'discoverDocs',
  'resolveDocsTopic',
  'setProviderApiKey',
  // Providers
  'getProviderModels',
  // Project
  'selectProjectFolder',
  'getProjectPreferences',
  'setProjectPreferences',
  'listProjectActions',
  'addProjectAction',
  'updateProjectAction',
  'deleteProjectAction',
  'discoverT3ProjectActions',
  'importT3ProjectAction',
  'listAuthorizationGrants',
  'grantAuthorization',
  'revokeAuthorization',
  'listSyntaxThemes',
  'selectSyntaxThemeImport',
  'applySyntaxThemeImport',
  'removeSyntaxTheme',
  'searchWorkspaceFiles',
  'searchWorkspaceContent',
  'cancelWorkspaceContentSearch',
  'readWorkspaceFile',
  'readWorkspaceFileWithEncoding',
  'readWorkspaceFilePage',
  'writeWorkspaceFile',
  'applyWorkspaceDocumentEdits',
  'listWorkspaceExternalEditors',
  'openWorkspaceFileExternal',
  'openAbsoluteFileExternal',
  'createWorkspaceEntry',
  'moveWorkspaceEntry',
  'duplicateWorkspaceEntry',
  'trashWorkspaceEntry',
  'revealWorkspaceEntry',
  'watchWorkspaceFiles',
  'unwatchWorkspaceFiles',
  'onWorkspaceFilesChanged',
  'listSessionsByIds',
  'listSessionCatalogPage',
  'listSessionProjectPage',
  'listHiveSessionCatalogPage',
  'getSessionDetail',
  'dismissInterruptedSessionRun',
  'listTurnCheckpoints',
  'getTurnDiffFiles',
  'getTurnDiff',
  'listPinnedSessions',
  'pinSession',
  'unpinSession',
  'movePinnedSession',
  'createSession',
  'forkSessionToNew',
  'cloneSessionToNew',
  'deleteSession',
  'archiveSession',
  'unarchiveSession',
  'updateSessionTitle',
  'setSessionAuthorizationMode',
  'listArchivedSessionBranches',
  'getSessionTree',
  'getSessionWorkspace',
  'listSessionResources',
  'listSessionResourcePage',
  'getSessionResource',
  'locateSessionResourceImage',
  'listSessionResourceNodePage',
  'listSessionResourcesByNodeIds',
  'advanceSessionResourceBackfill',
  'readSessionResource',
  'readSessionResourceThumbnail',
  'copySessionResourceImage',
  'prepareSessionResourceAttachment',
  'discardPreparedAttachment',
  'retrySessionResource',
  'recordSessionChangeRequest',
  'navigateSessionTree',
  'renameSessionBranch',
  'archiveSessionBranch',
  'restoreSessionBranch',
  'updateSessionTreeUiState',
  'onGitWorkingTreeChanged',
  'onGitStackedActionProgress',
  'onSessionTitleUpdated',
  'onSessionListInvalidated',
  // Terminal
  'getTerminalActivitySnapshot',
  'openTerminal',
  'detachTerminal',
  'clearTerminal',
  'restartTerminal',
  'closeTerminal',
  'resizeTerminal',
  'writeTerminal',
  'sendTerminalInputNow',
  'acknowledgeTerminalOutput',
  'assessTerminalClose',
  'migrateTerminalOwner',
  'onTerminalEvent',
  'onTerminalActivitySnapshot',
  // Browser preview
  'openBrowserPreview',
  'setBrowserPreviewBounds',
  'navigateBrowserPreview',
  'goBackBrowserPreview',
  'goForwardBrowserPreview',
  'reloadBrowserPreview',
  'stopBrowserPreview',
  'closeBrowserPreview',
  'replaceBrowserPreviewForCapacity',
  'zoomBrowserPreview',
  'setBrowserPreviewViewport',
  'hardReloadBrowserPreview',
  'setBrowserPreviewAppearance',
  'setBrowserPreviewAudioMuted',
  'openBrowserPreviewDevTools',
  'clearBrowserPreviewCookies',
  'clearBrowserPreviewCache',
  'captureBrowserPreviewScreenshot',
  'revealBrowserPreviewArtifact',
  'copyBrowserPreviewScreenshot',
  'startBrowserPreviewRecording',
  'saveBrowserPreviewRecording',
  'stopBrowserPreviewRecording',
  'pickBrowserPreviewElement',
  'cancelBrowserPreviewElementPick',
  'openBrowserPreviewPictureInPicture',
  'closeBrowserPreviewPictureInPicture',
  'registerBrowserPreviewOwner',
  'unregisterBrowserPreviewOwner',
  'acknowledgeBrowserPreviewOpenRequest',
  'setCurrentBrowserPreview',
  'respondBrowserPreviewRecordingRequest',
  'setBrowserPreviewShortcutBindings',
  'listBrowserImportSources',
  'importBrowserCookies',
  'guidedImportBrowserCookies',
  'openBrowserImportFullDiskAccessSettings',
  'clearBrowserProfileData',
  'onBrowserPreviewState',
  'onBrowserPreviewShortcut',
  'onBrowserPreviewKeyEvent',
  'onBrowserPreviewOpenRequest',
  'onBrowserPreviewOpenRequestCancellation',
  'onBrowserPreviewRecordingRequest',
  'onBrowserPreviewRecordingCancel',
  // Window
  'onFullscreenChanged',
  // Git
  'getGitStatus',
  'commitGit',
  'getGitDiff',
  'getGitBranchDiff',
  'stageAllGitChanges',
  'revertAllGitChanges',
  'listGitBranches',
  'checkoutGitBranch',
  'createGitBranch',
  'validateGitBranchName',
  'listGitWorktrees',
  'createGitWorktree',
  'removeGitWorktree',
  'getLocalVcsStatus',
  'getRemoteVcsStatus',
  'runStackedGitAction',
  'cancelStackedGitAction',
  'preflightChangeRequest',
  'listChangeRequests',
  'checkoutChangeRequest',
  'getChangeRequestPanel',
  'mergeChangeRequest',
  // Attachments
  'prepareAttachments',
  'prepareAttachmentFromText',
  'onPrepareAttachmentFromTextProgress',
  // Voice
  'transcribeVoiceLocal',
  // Standards & Skills
  'getStandardsStatus',
  'getEffectiveAgents',
  'listSkills',
  'setSkillEnabled',
  'getSkillPreview',
  // Shell / App
  'copyToClipboard',
  'readFromClipboard',
  'openLogsDir',
  'getLogsPath',
  'openPath',
  'revealPath',
  // Dialog
  'showConfirm',
  // Waggle
  'sendWaggleMessage',
  'cancelWaggle',
  'onWaggleEvent',
  'onWaggleTurnEvent',
  // Auth
  'startOAuth',
  'submitAuthCode',
  'cancelOAuth',
  'disconnectAuth',
  'getAuthAccountInfo',
  'onOAuthStatus',
  // Waggle presets
  'listWagglePresets',
  'saveWagglePreset',
  'deleteWagglePreset',
  // Feedback
  'checkGhCli',
  'collectDiagnostics',
  'getRecentLogs',
  'submitFeedback',
  'generateFeedbackMarkdown',
  'openExternal',
  // Composer
  'suggestFiles',
  // Auto-updater
  'checkForUpdates',
  'checkSessionWorktree',
  'installUpdate',
  'getUpdateStatus',
  'getAppVersion',
  'getDesktopNativeAdmissionIssue',
  'onUpdateStatus',
]
