...
describe('makePtyRunner', () => {
  ...
  it('spawns the first existing shell as a login shell on darwin and reports its name', async () => {
    ...
    const runner = makePtyRunner(async () => fromPartial<typeof NodePtyModule>({ spawn: spawnMock }))

    const outcome = await runner.spawn({ cwd: '/tmp/repo', cols: 100, rows: 28 } satisfies PtySpawnRequest)

    expect(outcome).toMatchObject({ ok: true, pid: 4321, shell: 'zsh' })
    expect(spawnMock).toHaveBeenCalledOnce()
    const [shell, args] = spawnMock.mock.calls[0] as [string, string[]]
