 ...
 * while the label promised a decision (#157). These drive real Git so the resolution order
 * (remote-tracking default, then local default, then no default at all) is exercised rather
 * than mocked.
 */
...
describe('Automatic base ref resolution', () => {
  it('diffs against the local default branch when there is no remote', async () => {
    const cwd = await createRepository()
    await git(cwd, ['checkout', '-b', 'feature'])
    await commitFile(cwd, 'committed-on-feature.txt', 'feature\n', 'feat: add feature file')
    // An uncommitted change must NOT appear: a branch diff is commits, not the working tree.
    await writeFile(path.join(cwd, 'uncommitted.txt'), 'scratch\n', 'utf8')

    const automatic = await getGitBranchDiff(cwd, '')

    expect(automatic.ok).toBe(true)
    expect(changedPaths(automatic)).toEqual(['committed-on-feature.txt'])
  })

  it('prefers the remote-tracking default branch over the local one', async () => {
    const cwd = await createRepository()
    ...
    try {
      ...
      // Advance local main only. origin/main stays at the baseline, so a diff resolved
      // against the remote-tracking ref includes this commit and a local-only one would not.
  ...
  it('falls back to the working-tree diff when no default branch exists', async () => {
    const cwd = await createRepository('trunk')
    // Point init.defaultBranch at a branch that was never created, so nothing resolves.
    await git(cwd, ['config', 'init.defaultBranch', 'nonexistent'])
  ...
  it('still honours an explicitly chosen base ref', async () => {
    ...
    expect(changedPaths(explicit)).toEqual(['on-feature.txt'])
  })
})
