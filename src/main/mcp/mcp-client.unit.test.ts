...
const mocks = vi.hoisted(() => {
  ...
  const getFullProcessEnvMock = vi.fn()
...
vi.mock('../env', () => ({
  getFullProcessEnv: mocks.getFullProcessEnvMock,
}))
...
describe('McpClient', () => {
  beforeEach(() => {
    ...
    mocks.getFullProcessEnvMock.mockReset()
    ...
    mocks.getFullProcessEnvMock.mockReturnValue({
      PATH: '/usr/bin',
      HOME: '/tmp/home',
    })
  ...
  it('connects a stdio server, merges env overrides, and hydrates tool metadata', async () => {
