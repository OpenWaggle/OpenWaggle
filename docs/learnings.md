...
- Waggle transcript attribution must be keyed by persisted message id and grouped by explicit turn metadata (`sessionId`, `turnNumber`, `agentIndex`). Visible assistant-message position is not stable because renderer projection can hide or nest tool-result nodes, which otherwise splits one logical agent turn into repeated headers and short color rails. [SKILL?]
...
- Durable active-run rows must survive Pi snapshot reprojection. Because `session_active_runs.branch_id` references `session_branches`, snapshot rebuilds that delete/reinsert branches need to preserve and reinsert compatible active-run rows before marking interrupted state; otherwise restart reconciliation silently loses the indicator. [SKILL?]
...
- Files under `src/shared` are compiled into both Node and renderer TypeScript projects. Keep Node built-in imports such as `node:path` and `node:fs` out of shared utilities; put Node-specific helpers under `src/main/...` and keep shared modules limited to platform-neutral types, constants, schemas, and pure logic.
- `tsx`-run preflight scripts execute before Electron/Vite bundling and can resolve ESM-only packages through CommonJS paths. Keep `@diegogbrisa/ts-match` imports out of pre-start scripts such as `scripts/rebuild-native-deps.ts`, or verify the package exposes compatible `require` conditions before using it there. [SKILL?]
...
- OpenWaggle's Electron renderer uses TanStack `createHashHistory()`. Live CDP/Playwright QA must navigate to `http://localhost:5173/#/<route>` instead of plain pathname routes; changing only `pathname` can leave the router outlet on the previous rendered surface while the browser URL appears to change. [SKILL?]
