...
The Summary requests 50 Workers per page and loads subsequent pages only when requested. It resolves the current Session by identity and rejects explicitly mismatched Worker parent IDs. When a parent ID is omitted, the Host's scoped `workers` and immediate-parent `context` are authoritative; omitted metadata must not reject a valid page or hide parent navigation. Explicit `null` still means no parent, and ambiguous context with multiple possible parents is rejected. A single parent context row must also agree with an explicit parent ID; a contradictory row, including one supplied for an explicit-null root, produces a recoverable Hive error instead of silently hiding parent navigation. The Summary deduplicates Workers across page boundaries. A session-keyed query prevents a late page from appearing after navigation to another Session.
