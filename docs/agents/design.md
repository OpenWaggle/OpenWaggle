...
- Show one active workspace file. Keep one shared, collapsible workspace navigator on the right in both file and diff review; preserve its width and open state across those surfaces. Keep Quick Open, project text search, Go to Line, and external-editor handoff; do not add tabs, split panes, editor history, or hidden document models.
