...
`pnpm benchmark:syntax` owns deterministic microbenchmarks and checks the controlled desktop profile by default. GitHub's three-core shared macOS runner uses a separate regression profile with strict median budgets and bounded p95 scheduler headroom; it runs on the non-gating nightly packaged canary (`.github/workflows/nightly.yml`) rather than as a merge gate (ADR 0033 removed the macOS E2E job that previously hosted it), so it catches sustained slowdowns without blocking merges or treating one shared-runner scheduling outlier as an app regression. Bundle-shape tests and component tests cover properties that a worker microbenchmark cannot establish. A future branch-vs-base CI comparison must run both revisions on the same controlled runner before it can be used as a relative merge gate. Absolute timing claims come only from controlled desktop traces.
