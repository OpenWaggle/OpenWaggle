...
- Recovery dispatches the Package Release workflow from `main` with the exact merged release commit SHA. The workflow verifies that commit is reachable from `origin/main`, derives its first parent, matches its Git tree to the successful attested release-candidate artifact, and resumes only missing package versions whose registry integrity still matches. Recovery never rebuilds or replaces an existing version.
