...
The workflow never merges its own release pull request. A maintainer's protected merge starts publication only after the merged tree is proven identical to the exact CI-validated release-PR tree and its parent is proven identical. It then checks out that trusted parent, installs the parent's frozen dependencies, regenerates with the parent's release generator and commit date, and requires an exact Git tree match, including file modes, before tagging. Publication pushes only the tag, builds platform artifacts, verifies installers, and publishes `.release/release-notes.md` verbatim with checksums. Preparation runs share a coalescing concurrency group, while every protected-merge publication run is keyed by its immutable merge SHA. Ambiguous pushes and pull-request creation calls adopt only exact same-repository state; incompatible state, exhausted validation retries, and a preexisting next-version tag that does not belong to the exact protected merge fail closed.
...
The Release Please pull request is the final release candidate. Its green gate builds each final-version tarball once, records the Git tree identity and SHA-256 digest, and uploads the immutable artifacts with GitHub provenance. The repository pins the same Release Please runtime bundled by the immutable action revision and preflight-tests its generated changelog shape and coordinated PR-title policy. Merging that pull request is an explicit maintainer decision; release pull requests are never auto-merged and repository rules have no routine release bypass.
