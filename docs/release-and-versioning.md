...
- Direct package release intent is path-scoped. Release Please considers only release-eligible Conventional Commits that touch `packages/<name>/**`; app, website, general documentation, fixture, and workflow-only changes do not release npm packages.
...
- Publish validation builds packages, checks every documented export boundary, checks public API snapshots, enforces a strict tarball allowlist, smoke-installs packed tarballs through npm, pnpm, Yarn, and Bun on Node 22.19+ and Node 24, verifies unpublished versions, and publishes the exact validated tarballs.
...
- The additive `main` ruleset requires pull requests and green CI, allows only squash and rebase, blocks force pushes and deletion, and retains only an administrator emergency bypass. The app release workflow creates a version PR, reruns its approval-required PR-associated CI for the exact head, waits for that run, and merges through the normal ruleset before tagging the protected commit. Release Please package PRs use the same PR-associated rerun when available and exact-head `workflow_dispatch` when GitHub suppresses the bot-created PR event. Repository settings also disable merge commits so GitHub cannot synthesize a package-changing merge subject that passes PR checks but fails the commit policy on `main`. This uses only scoped `GITHUB_TOKEN` permissions and requires neither a PAT nor a GitHub App key.
