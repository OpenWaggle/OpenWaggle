...
After merge, publication must not rebuild, regenerate docs, or rerun quality checks. The publish workflow resolves the successful release-candidate artifact for the merged Git tree, verifies its provenance, tree identity, digest, package/version plan, OIDC identity, dependency availability, and unpublished npm state, then publishes that exact tarball through npm Trusted Publishing. Transient registry/network failures may retry and resume; deterministic content failures belong in the pre-merge gate.

Publish bases before dependents. After npm accepts and serves the exact version, create the immutable package tag and only then publish the matching GitHub Release. A GitHub Release must never claim availability before npm does.

The website guide under `website/src/content/docs/packages/<package>/<major>.<minor>/` is the canonical authored package documentation. `pnpm package-docs:update` generates committed package READMEs and current API-reference pages; `pnpm check` fails on drift. Package READMEs are self-contained npm landing pages with four tested package-manager commands and absolute documentation/support links. Installed agent docs expand website-only components into ordinary Markdown from the same source.
