...
If the renderer crashes or loses trust, retained previews are quarantined immediately. They are hidden, detached and muted, with detached DevTools closed. Normal lookup, cached automation, queued input and capture results cannot use the retired preview. Navigation, popup and security-prompt restrictions remain until native destruction. The main process owns cleanup retries with capped backoff, including when no explicit user close preceded retirement. Cleanup does not depend on a request from the crashed or untrusted renderer. Actual destruction cancels retry timers and releases the retained record. An ordinary failed user close does not quarantine the preview and remains usable for retry.

If a Session's last browser closes after the user switches elsewhere, its retained side terminal becomes that Session's selected panel without claiming the current Session's sidebar. Returning to the original Session restores the terminal. Explicitly hiding the panel while close is pending must not reopen it.
...
Concurrent Git status and branch-list requests for the same path share their pending read for at most 30 seconds. After that, a new refresh can start independently of a stalled child; the expired read cannot overwrite newer cached status. Status retains its existing successful-result TTL; branch lists are not retained after completion. Mutation invalidation separates new requests from old work and prevents stale status from repopulating the cache, including changes through subdirectory aliases. Branch reads are invalidated across linked worktrees because their refs are shared. Different paths never share a result.
