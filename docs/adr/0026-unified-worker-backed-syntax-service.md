...
**One renderer syntax service owns shared infrastructure.** The service owns the canonical language registry and aliases, normalized theme registry, lazy language/theme loading, at most two lazy workers, request priority, cancellation, bounded caches, and structured failure reporting. The bounded second lane prevents a large source request from starving compact visible code. It consumes only validated OpenWaggle theme packages.
