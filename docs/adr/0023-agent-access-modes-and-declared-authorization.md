...
- Before a session exists, the composer access menu is editable and may hold an explicit draft override. First send creates the session, persists that choice as a session override, and waits for the write before launching the task. An untouched draft keeps inheritance and stores no mode.
...
- The composer uses a custom menu rather than a native select. Its closed trigger presents the effective mode compactly as `YOLO` or `Ask for approval`; the open menu exposes exactly two choices with the canonical names `YOLO (Full Access)` and `Ask for Approval`. Inheritance stays internal, and the menu marks the effective choice without adding a third "Default" option.
...
- **Swapping compact and full labels inside a native select.** Rejected because it relied on Chromium repainting `<option>` text before the native popup opened; losing that race could show duplicate labels with no way to distinguish inheritance from an explicit override, and the native popup is outside the DOM assertions can inspect. The custom composer menu now makes the compact trigger and canonical menu labels separate, stable DOM content. The ambiguous contraction `Ask` remains rejected; the compact trigger says `Ask for approval`.
...
- First-send orchestration must persist an explicit draft override before task dispatch, while leaving an inherited draft unset; displaying the effective default is not evidence that the user selected it.
