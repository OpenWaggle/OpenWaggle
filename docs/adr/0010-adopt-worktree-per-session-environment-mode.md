...
Status: accepted; one-session/one-worktree ownership cardinality superseded by ADR 0030

ADR 0030 replaces the ownership unit with a shareable Workspace resource. The local-versus-worktree environment distinction, managed storage, git-native dirty-removal protection, and no-worktree behavior for non-Git projects remain in force; session-exclusive worktree identity and cleanup do not.
