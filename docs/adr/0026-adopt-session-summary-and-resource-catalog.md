...
- Hive shows only the opened Session's immediate parent and direct Workers, groups Workers as Active, Review, Done, and Archived, and keeps the agreed per-session expansion behavior. A submitted Worker waiting for its parent to accept or request revision is in Review, not Active.
