...
The remodelled sidebar needed nine states to carry colour. Nine call sites reaching for palette classes would have made the bypass permanent, and it would have pinned the product's status vocabulary to one palette.
...
**`--color-error` needs a partner for text.** `#ef4444` measures 4.49:1 on `--color-bg-secondary`. That clears the 3:1 bar WCAG sets for icons, borders and other non-text, and misses the 4.5:1 bar for body text by a hundredth. The row's 10.5px state label would therefore have failed AA. Rather than lighten the role everywhere and lose the app's existing error colour, the label uses `--color-error-text` at 6.1:1. This is one exception, not a `<role>-text` pattern: every other role clears 4.5:1 unaided, and inventing the pattern for a single case would be speculative structure.
...
**The theme block is `@theme static`.** Tailwind tree-shakes theme variables that no utility class references, and three roles are read at runtime through `var()` in an inline style rather than through a class: `--color-neutral`, `--color-review` and `--color-plan`. Without `static` they were declared and absent from the stylesheet, so an idle row's `--row-color: var(--color-neutral)` resolved to nothing and its glyph fell back to inherited colour. Verified with the Tailwind CLI: a plain `@theme` emitted `--color-progress`, used by a class, and dropped `--color-review`, which is not.
