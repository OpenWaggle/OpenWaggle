...
- The initial health deadline is armed before frame navigation, and readiness is accepted only after fragment parsing reaches `DOMContentLoaded`. A low-frequency heartbeat detects later silent navigation or hangs. Visualization origins run out of process; expiry removes the isolated iframe from the responsive application renderer without targeting an operating-system process.
