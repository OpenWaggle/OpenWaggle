...
- The bundled authoring skill is always discoverable in the Slash command menu as `/visualize`. Choosing it inserts the normal `visualize` skill mention; it does not create a second command execution path or let the renderer author transcript content.
...
- The sandbox uses a deny-by-default CSP. Arbitrary connections are blocked; `connect-src` is limited to `blob:` and `data:`. Workers, frames, objects, base-URI changes, and form submission are blocked. Unknown messages and host calls are ignored or rejected.
...
- Expanding a wide visualization opens a Visualization focus layer above the complete Electron app, not a wider card inside the transcript. The same mounted frame is promoted in place without reparenting, top-layer transitions, or frame navigation, so local interaction state survives expansion and collapse. The layer owns modal focus containment, Escape and backdrop dismissal, safe viewport insets, and its own overflow.
...
- A fragment may call `window.openai.setVisualizationState` with a bounded JSON value when its meaningful selection, filters, or controls change. The authenticated host keeps only the latest mounted Inline visualization state snapshot per active session and source. The next user-authored turn includes the most recently updated mounted snapshot as delimited, untrusted user context; unmounting or explicitly clearing the state removes it. The host never scrapes DOM state, treats a snapshot as instructions, or grants the frame a read path into the session.
...
- The initial health deadline is armed before frame navigation. The injected head runtime sends a credential-bearing bootstrap before fragment parsing; that first bootstrap authenticates the frame but does not clear the deadline. Readiness is accepted only later, with the same credential, after fragment parsing reaches `DOMContentLoaded`. A low-frequency authenticated heartbeat detects later silent navigation or hangs. Visualization origins run out of process; expiry removes the isolated iframe from the responsive application renderer without targeting an operating-system process.
...
- Source-session attribution grants read ownership only. A confirmed visualization follow-up always targets the session in which the user is currently interacting, so a copied, forked, or handed-off visualization cannot enqueue work under the source session's authorization or worktree.
...
- Renderer-process isolation is a liveness and resource-containment boundary, not the authority boundary. OpenWaggle enables Chromium site-per-process isolation so a parser-blocking fragment cannot freeze the transcript and can be stopped by removing its iframe, while sandboxing, origin separation, CSP, and brokers remain the security controls. OpenWaggle does not send OS-process kill signals for frame recovery because Electron may surface a child-renderer crash as a crash of the containing page on some platforms.
- The trusted frame runtime enforces a rolling main-thread budget and removes a visualization when browser long-task accounting exceeds one second in five seconds. Workers are disabled, so responsive fragments cannot move unbounded work outside that measured frame thread. A runtime without the required accounting API fails closed.
...
- Accepted wire forms are, deliberately: the documented bare `visualize{"path":…}` line (canonical, model-reproducible; detected alone on its own line with a strict payload parse) and the legacy private-use-delimited form (`U+E200 visualize U+E202 {payload} U+E201`, matchable anywhere) retained for existing sessions. This dual acceptance, recorded 2026-09 after authoring models proved unable to reproduce invisible delimiters (issue #199), is the deliberate compatibility decision this ADR requires; the bare form's own-line rule and strict payload parse keep the false-positive surface at deliberate reference emissions only.
