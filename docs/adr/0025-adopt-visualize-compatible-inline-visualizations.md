...
- Renderer-process isolation is a liveness and resource-containment boundary, not the authority boundary. OpenWaggle enables Chromium site-per-process isolation so a parser-blocking fragment cannot freeze the transcript, while sandboxing, origin separation, CSP, and brokers remain the security controls.
