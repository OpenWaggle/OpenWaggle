...
**Fall back to Portable compaction after any declared Native compaction request fails.** Codex chooses the mechanism from provider capability before the attempt. Transport retries remain within that mechanism; authentication, quota, cancellation, malformed output, and exhausted transient failures surface as failed compaction and leave the current projection intact. Standard `404 Not Found`, `405 Method Not Allowed`, and `501 Not Implemented` responses are narrower: they disprove the endpoint capability declaration, so the request continues through Portable compaction as unsupported rather than as a failed supported request.
...
- A supported Native compaction attempt uses the transport's normal retry policy. Exhausted retries or malformed output leave the previous active projection installed and report failure; they do not silently select Portable compaction. Standard endpoint-unavailable responses (`404`, `405`, or `501`) correct a stale capability declaration and continue through Portable compaction without a provider-specific branch. If the active projection is an opaque Native checkpoint, the Portable preparation reconstructs append-only raw history first so the fallback does not collapse older context into a placeholder label.
