...
- Known OpenWaggle `pi-mcp-adapter` files, package artifacts, constants, version checks, package-copy behavior, Pi-setting mutations, UI wording, and tests are removed in the clean migration. The exact OpenWaggle-owned `extensions/pi-mcp-adapter` package entry is removed from Pi settings. User-managed `pi-mcp-adapter` npm entries remain installed and unchanged for standalone Pi and other projects, but OpenWaggle masks them from its embedded Pi runtime because OpenWaggle owns MCP directly. The runtime-only mask must restore those entries on any settings write. Legacy global, Pi, `.agents`, project OpenWaggle, and disabled-server configuration is reported through preview-first import. Plaintext credentials discovered during import require explicit user review.
...
- `sessions:message` permits starting and queuing work; `sessions:steer` permits in-run steering without interruption; `sessions:interrupt` permits cancellation; and `sessions:queue` permits follow-up inspection and mutation. Replacement requires both message and interrupt authority, queued-content reads additionally require `sessions:read`, and `sessions:organize` remains limited to session metadata and worktree organization. `sessions:export` permits delivery but grants no content by itself: transcript export additionally requires `sessions:read`, queued content requires `sessions:queue`, included resources require their resource-read grants, and writing a destination remains subject to filesystem authorization.
- `sessions:respond` permits authorized non-approval interaction responses, while `sessions:approve` is a separate explicit delegation for Authorization decisions. A requesting agent cannot use request ownership as approval authority.
