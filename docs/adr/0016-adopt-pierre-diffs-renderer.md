...
- Exposes 23 `--diffs-*-override` CSS variables for the diff chrome, so our tokens drive appearance (see ADR 0015 amendment).
...
**Diff body: `@pierre/diffs@1.3.2`.** Render via `CodeView` from the unified patches we already produce. Delete the handmade renderer and the unified-diff parser it fed. Review comments move onto annotations (our `InlineComment` mounts inside the annotation slot) and CodeView line selection. The diff body mounts one lazy Pierre worker only while it is active, so parsing and syntax tokenization do not monopolize the renderer thread. Its AST cache is bounded and patch text is hashed once per file when building cache identities.
...
**Amendment: workspace documents use the Pierre renderer.** The unified syntax-highlighting work replaces the workspace file editor's plain `<textarea>` with the virtualized source view and focused editor recorded in ADR 0028. Pierre owns diffs, read-only source, and focused single-file editing. These surfaces also share language identity, TextMate grammars, and Syntax themes with the other syntax adapters.
...
The renderer becomes a maintained dependency instead of ~640 lines we own. We gain word-level diffs, syntax highlighting, split view, wrap, virtualization, and synchronized scrolling essentially for free, and the review-comment affordances the panel's purpose demands. A production Electron performance test holds a large multi-file diff to immediate shell feedback, bounded first-code latency, at most one Pierre worker, and no renderer task longer than 50 ms.
...
The Syntax theme sits outside the Design token contract by design (ADR 0015 amendment). A future reader must not "fix" that by folding syntax scopes into Semantic roles.

Workspace editing consequently becomes part of Pierre's dependency boundary and the shared syntax contract. See ADR 0028 for the editing, performance, and persistence decision.
...
**Adopt the `pierre-dark` theme wholesale for chrome too.** Rejected — see ADR 0015 amendment. Its blue selection/focus against our amber accent would make the review surface an off-brand island, and would contradict ADR 0015 two days after it was accepted.
