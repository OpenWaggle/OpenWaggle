...
Use this skill when:
- Image/PDF attachments create renderer memory spikes.
- IPC payloads include large base64 blobs.
- Conversation list refreshes parse every conversation file and degrade performance.
- You need a type-safe boundary between persisted attachment records and runtime transport payloads.
...
- Right before `runAgent` / multi-agent execution, resolve file paths and read image/pdf bytes.
...
- Fallback to full scan only when missing/corrupt, then rebuild index.
