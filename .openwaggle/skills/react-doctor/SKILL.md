...
{
  "ignore": {
    "files": [".claude/worktrees/**", ".codex/worktrees/**"],
    "rules": []
  }
}
```
...
Per project conventions in `CLAUDE.md`: never use `React.memo()`, `useMemo()`, or `useCallback()` for render optimization — the React Compiler handles memoization automatically.
