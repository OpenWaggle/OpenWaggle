...
The v0.1 ships with: voice input (Whisper), OCR (tesseract.js), DOCX parsing (mammoth), PDF extraction (pdf-parse), xterm terminal, diff panel, skills system, orchestration pipeline, and 6 LLM providers. Each is a maintenance surface. None of them are the product differentiator (multi-agent).
