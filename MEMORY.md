...
- A parser-blocking visualization can prevent `load` entirely, so its deadline must be armed before assigning `src`; the trusted runtime must not report ready until fragment parsing reaches `DOMContentLoaded`. Keep visualization sites out of process, use a low-frequency heartbeat for later silent navigation, and terminate only a registered renderer process proven distinct and unshared with the app.
