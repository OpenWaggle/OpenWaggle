...
- Optimistic per-session model picks need a generation-stamped desired-pick guard (`src/renderer/src/shared/lib/session-model-pick.ts`): every detail/summary write reconciles through it, per-session model writes serialize through one queue, failed picks restore the last committed generation, and every dispatch path (send gate, compaction, fork/clone, branch switch, hosted tasks, transcript retries) validates selectability against the hydrated Pi catalog — a non-empty stored model string alone is not proof the model still exists.
