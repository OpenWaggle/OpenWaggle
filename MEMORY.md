...
- Framed extension actions should preserve their mounted DOM while an asynchronous broker call settles. Replacing the frame root from inside the initiating click callback can erase completion feedback under Linux/Windows CI timing; update the existing controls and status node in place, and cover node identity plus the real Electron frame flow.
