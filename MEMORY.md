...
- Persisted Session draft keys use the matching workspace's canonical nullable project path, not asynchronously updated global project preferences. Otherwise a late preference update can clear text after the composer is already editable. Branch-summary restoration and completed-source cleanup must use that same nullable key. Only new-Session drafts follow the global selected project.
