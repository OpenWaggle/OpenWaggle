...
- Pierre patch parsing is synchronous even when highlighting uses its worker pool. Prepare ordinary multi-file patches in renderer tasks bounded by both file count and UTF-16 input units, publishing incremental items so the first highlighted file is not blocked by the whole change set. Parse any single oversized unified patch in a short-lived module worker and terminate it before mounting Pierre's highlighting worker; retain and replay Changed-file navigation until the requested item has been prepared.
- Pierre renders highlighted rows inside the open shadow root of its `<diffs-container>` element. Readiness probes must observe that shadow root rather than relying on a light-DOM `querySelector`, and the expanded Changed-file navigator must receive large file lists through deferred rendering so hundreds of secondary tree rows cannot delay the primary loading/highlight frame.
- Pierre's working pool owns the effective focused-editor render options. Passing the active Syntax theme only to `<File>` is insufficient and falls back to Pierre's default theme when Edit opens; initialize the worker pool and file with the same revision-specific runtime theme.
