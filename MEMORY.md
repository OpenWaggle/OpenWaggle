...
node-pty 1.1.0 never resumes its master-side `tty.ReadStream` after spawn; whether a paused fd-backed stream starts flowing is runtime-dependent (plain Node flows, Electron's build does not), so inside Electron the shell's first output batch (the prompt) sat in the kernel buffer until a later write kicked it — dead empty panes. Fix: `resumeMasterStream` in the pty runner resumes the `_socket` via runtime guards. Separately, xterm's WebGL addon mis-scales glyphs inside the panel (DPR/viewport mismatch: content renders tiny in the top-left corner) — the DOM renderer is deterministic, so WebGL is dropped (ADR 0030 decision 4); do not re-enable without in-app visual validation. Component tests mock the addons and cannot see either bug; both were only visible in the real Electron app.
Terminals are keyed `(ownerKey, terminalId)` where ownerKey is the session id or `draft:<projectPath>`; the shell cwd is the session's Working path (`resolveSessionWorkingDir`), never the raw project path. Main-process ownership lives in `makeNodePtyTerminalService` -> `terminal-runtime.ts` (registry + 10ms coalesced output + spawn generation guards) -> `terminal-service-actions.ts` (pure-ish open semantics mirroring t3code's re-open: reuse on same cwd, kill+reset on changed cwd, respawn-on-demand when dead). Scrollback persists to `userData/terminal-logs/<base64url(key)>.log` via a coalescing writer (40ms) that compacts to the newest 5,000 lines once appends exceed 125% of the cap — without compaction the file grows unbounded (caught by the throughput benchmark). Persisted chunks must be sanitized (strip DSR/DA/DECRQM/XTVERSION/kitty/OSC-10-11-12 queries) or replay re-triggers terminal queries; the sanitizer carries incomplete escape tails across chunks. Renderer: `terminal-store` (zustand persist, sanitized rehydrate) owns per-owner tab/pane layout; panes detach (`terminal:detach`) on unmount instead of closing, and attach replays the main-process scrollback, so hide/reload/restart all restore visual state. Events go only to attached WebContents (never broadcast). E2E gotcha: headless Electron gets WebGL (SwiftShader), so xterm renders to canvas and `.xterm-rows` text is absent — force the DOM fallback by stubbing `HTMLCanvasElement.prototype.getContext` to null in `addInitScript`. macOS hotkey gotcha: Playwright `Meta+J` (uppercase) injects Shift and misses bindings; use lowercase `+j`. Throughput reference (`pnpm tsx scripts/benchmark-terminal-output.ts`): 3.4MiB flood from `yes|head -n 200000` collapses into ~25 IPC events (~8k lines/event), attach ~5ms.
...
- Diff performance E2E brackets the sample immediately before the panel toggle and ends it when the light-DOM `data-diff-code-ready` contract reflects Pierre's shadow-root readiness. Long-task samples retain `startTime` and are filtered to that window. A body-level code query cannot see Pierre's shadow rows, and falling back to the time when Playwright finishes polling charges startup, runner descheduling, and test-driver delay to diff rendering.
