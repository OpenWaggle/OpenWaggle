...
- Responsive sidebar mode changes must keep the main React subtree mounted. Switching its parent between a sheet fragment and a docked layout destroys Lexical chips, unsent text, selection, and embedded visualization state. Change only the sidebar presentation inside a stable frame.
