...
Before starting a Project Action in a managed worktree, compare the checkout's current directory identity with its saved preparation generation. A worktree recreated at the same path outside OpenWaggle can otherwise reuse Setup exports from the removed checkout. Reject the launch before handing those exports to the action runner; a worktree with no preparation snapshot remains launchable. POSIX Setup capture must also intercept escaped `\eval` (including dynamic nested eval) before its text can reach an escaped `\exec` and replace the shell without saving new exports. Test the capture behavior across the installed POSIX shells.
