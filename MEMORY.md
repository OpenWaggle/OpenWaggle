...
Windows native final-output probes must account for the console host's VT presentation controls and row wrapping. Normalize those controls before requiring the exact 256 KiB payload and both boundary markers; still reject missing, duplicated, or changed payload characters. Keep Unix output byte-exact. Never classify raw ConPTY stream length alone as data corruption.
