...
### t3code-style release pipeline (ADR 0033, September 2026, supersedes ADR 0029)

Releasing one feature drove the full test matrix ~5x: PR run + merge-queue `merge_group` run + post-merge `push: main` run + release-please `chore(release)` PR run + post-merge run when that release PR landed. Adopted `pingdotgg/t3code`'s model instead: `ci.yml` on `pull_request` + `push: main` only (no merge queue), three jobs — `Check` (`pnpm check`), `Test` (`pnpm test:unit && :integration && :component`), `Release Smoke` (`pnpm build`); one PR run + one main run per feature. `release.yml` is decoupled — triggers on a pushed `v*` tag (stable), a nightly `schedule` cron (guarded by `check_changes` vs the last `v*-nightly.*` tag), or `workflow_dispatch`; job graph `check_changes → preflight → quality → build (mac/linux/win electron-builder, no signing, `CSC_IDENTITY_AUTO_DISCOVERY: false`) → release (softprops action-gh-release) → finalize`. Version comes from the tag/dispatch input (stable) or a computed `-nightly.<date>.<run>` (nightly), replacing release-please for the app. Deliberate deviations from t3code: no code signing/notarization; no `queue: max` (non-standard GitHub Actions field); dropped their Rust/mobile/server/relay/CLI/AUR/web-deploy/Discord jobs (OpenWaggle has no analogue); mac arm64+x64 built in one electron-builder call (so no updater-manifest merge). `finalize` pushes the stable version bump to `main` with the built-in `GITHUB_TOKEN` (no GitHub App), which needs a `github-actions` (app id 15368) bypass actor on the `main` ruleset. Adopting the pipeline requires a maintainer settings change: drop the `merge_queue` rule and swap required checks to `Check`/`Test`/`Release Smoke`; until then PRs show forever-pending old required checks. gh CLI cannot create a GitHub App or mint an App/PAT token (browser-only); it can only store an existing token or edit the ruleset via `gh api`.
