...
- **React Compiler runs in the app build (`electron.vite.config.ts` -> `reactCompilerPreset()`) but NOT under vitest.** Any component that reads render data from a library-owned *mutable* instance can therefore pass every test and render permanently stale in the real app: the compiler memoizes on referential identity, and the instance is mutated in place so its identity never changes. Hit for real with `@headless-tree` in the Changed-file navigator — `tree.getItems()` returned 262 items while zero rows reached the DOM. Fix is the scoped `'use no memo'` directive on the component that maps the mutable instance (an official compiler escape hatch, not a lint-ignore comment). Treat a green suite as **no evidence** about this class of bug; verify such components in real Electron over CDP.
