...
- macOS hidden iframe clicks can also miss intermittently despite a native presentation subscription. For non-privileged tab changes, use the same delegated DOM click on every platform and assert its synchronous selected state. Keep native keyboard navigation, focus assertions, and trusted activation checks for privileged follow-up actions; synthetic clicks must not stand in for those security tests.
