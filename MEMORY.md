...
- The shared syntax service is the review-surface admission and scheduling boundary. It starts at most one worker after syntax-eligible content becomes visible, prioritizes visible work, bounds caches, cancels superseded requests, and quarantines repeatedly failing imported grammars. Focused file edit may mount one separate Pierre worker only for that explicit editor's lifetime. Both worker builds stay module-split so grammars load on demand. Cancellation is normal control flow and never counts as a grammar failure.
